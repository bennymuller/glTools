...
def getDeformerList(nodeType='geometryFilter', affectedGeometry=[], regexFilter=''):
    ...
    if affectedGeometry:
        ...
        historyNodes = cmds.listHistory(affectedGeometry, groupLevels=True, pruneDagObjects=True)
...
def findInputShape(shape):
    ...
    inMeshConn = cmds.listConnections(shape + '.inMesh', source=True, destination=False, shapes=True)
    if not inMeshConn:
        return shape
    ...
    if not deformerObj.hasFn(OpenMaya.MFn.kGeometryFilt):
        ...
        if not deformerHist:
            print(
            'findInputShape.py: Shape node "' + shape + '" has incoming inMesh connections but is not affected by any valid deformers! Returning "' + shape + '"!')
            return shape
...
def renameDeformerSet(deformer, deformerSetName=''):
    ...
    # Rename deformer set
    deformerSet = cmds.listConnections(deformer + '.message', type='objectSet')[0]
    if deformerSet != deformerSetName: deformerSetName = cmds.rename(deformerSet, deformerSetName)
...
def bindPreMatrix(deformer, bindPreMatrix='', parent=True):
    ...
    cmds.xform(bindPreMatrix, ws=True, matrix=cmds.xform(deformerHandle, q=True, ws=True, matrix=True))
    cmds.xform(bindPreMatrix, ws=True, piv=cmds.xform(deformerHandle, q=True, ws=True, rp=True))

    # Connect inverse matrix to localize cluster
    cmds.connectAttr(bindPreMatrix + '.worldInverseMatrix[0]', deformer + '.bindPreMatrix', f=True)

    # Parent
    if parent: cmds.parent(deformerHandle, bindPreMatrix)
...
def checkMultipleOutputs(deformer, printResult=True):
    ...
    for i in range(numIndex):
        plugConn = cmds.listConnections(deformer + '.outputGeometry[' + str(indexList[i]) + ']', s=False, d=True, p=True)
        ...
        if len(plugConn) > 1:
            ...
            if printResult:
                print('Deformer output "' + deformer + '.outputGeometry[' + str(indexList[i]) + ']" has ' + str(
                    len(plugConn)) + ' outgoing connections:')
