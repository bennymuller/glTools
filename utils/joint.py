...
def getJointList(startJoint, endJoint):
    ...
    # Check hierarchy
    decendantList = cmds.ls(cmds.listRelatives(startJoint, ad=True), type='joint')
    ...
    while (jntList[-1] != startJoint):
        pJnt = cmds.listRelatives(jntList[-1], p=True, pa=True)
...
def group(joint, indexStr='A'):
    ...
    # Check Index String
    if not indexStr:
        result = cmds.promptDialog(title='Index String',
        ...
        if result == 'Create':
            indexStr = cmds.promptDialog(q=True, text=True)
        else:
    ...
    grp = cmds.duplicate(joint, po=True, n=prefix + 'Con' + indexStr + '_jnt')[0]
    ...
    inverseScaleCon = cmds.listConnections(joint + '.inverseScale', s=True, d=False)
    ...
    # Delete User Attrs
    udAttrList = cmds.listAttr(grp, ud=True)
    if udAttrList:
...
def setDrawStyle(joints, drawStyle='bone'):
    ...
    for jnt in joints:
        ...
        # Bone
        if drawStyle == 'bone': cmds.setAttr(jnt + '.drawStyle', 0)
        # Box
        if drawStyle == 'box': cmds.setAttr(jnt + '.drawStyle', 1)
        # None
        if drawStyle == 'none': cmds.setAttr(jnt + '.drawStyle', 2)
...
def duplicateChain(startJoint,
    ...
    skipJoints = cmds.ls(skipJnt) if skipJnt else []
...
def flipOrient(joint, target='', axis='x'):
    ...
    cmds.setAttr(target + '.jo', flipRot.x * radToDeg, flipRot.y * radToDeg, flipRot.z * radToDeg)
...
def connectInverseScale(joint, invScaleObj=None, force=False):
    ...
    if not invScaleObj:
        ...
        if parent and force: parent = cmds.ls(parent, type='joint') or []
        if not parent:
    ...
    invScaleCon = cmds.listConnections(joint + '.inverseScale', s=True, d=False) or []
    if not invScaleObj in invScaleCon:
        try:
            cmds.connectAttr(invScaleObj + '.scale', joint + '.inverseScale', f=True)
        except Exception, e:
            print(
            'Error connecting "' + invScaleObj + '.scale" to "' + joint + '.inverseScale"! Exception msg: ' + str(e))
