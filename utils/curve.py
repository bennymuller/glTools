...
def createFromPointList(pointList, degree=3, prefix=''):
    ...
    # Build curve
    crv = cmds.curve(p=cvList, k=range(len(cvList)), d=1)
    crv = cmds.rename(crv, prefix + '_crv')
...
def createFromLocators(locatorList, degree=3, attach=False, prefix='curve'):
    ...
    for i in range(len(locatorList)):
        locPosList.append(cmds.getAttr(locatorList[i] + '.worldPosition')[0])

    # Build curve
    curve = createFromPointList(locPosList, degree, prefix)

    # Rename Curve
    curveShape = cmds.listRelatives(curve, s=True, pa=True)[0]
    curveShape = cmds.rename(curveShape, curve + 'Shape')
...
def locatorCurve(curve, controlPoints=[], locatorScale=0.05, local=False, freeze=True, prefix='', suffix='loc'):
    ...
    curveShape = None
    if cmds.objectType(curve) == 'transform':
        curveShape = cmds.listRelatives(curve, s=1, ni=1)[0]
    else:
        curveShape = curve
        curve = cmds.listRelatives(curve, p=1)[0]
    ...
    # Set Locator Scale
    locatorScale *= cmds.arclen(curve)
    ...
    for i in controlPointList:
        ...
        # Get CV Position
        pos = cmds.pointPosition(curve + '.cv[' + str(i) + ']')

        # Create Locator
        locator = cmds.spaceLocator(n=prefix + '_cv' + ind + '_' + suffix)[0]

        # Position Locator
        cmds.xform(locator, ws=True, t=pos)
        cmds.setAttr(locator + ".localScale", locatorScale, locatorScale, locatorScale)

        # Add CV ID Attribute
        cmds.addAttr(locator, ln='cvID', at='long', dv=i)

        # Connect to CV
        if local:
            cmds.connectAttr(locator + '.translate', curve + '.controlPoints[' + str(i) + ']')
        else:
            if freeze: cmds.makeIdentity(locator, apply=True, t=1, r=1, s=1, n=0)
            cmds.connectAttr(locator + '.worldPosition[0]', curve + '.controlPoints[' + str(i) + ']')
...
def locatorEpCurve(curve, locatorScale=0.05, prefix=''):
    ...
    openCrv = not cmds.getAttr(curve + '.form')
    numPt = spans + openCrv
    ...
    for i in range(numPt):
        ...
        curveCon = cmds.pointCurveConstraint(curveShape + '.ep[' + str(i) + ']', ch=1, rpo=1, w=1)
        locatorList.append(curveCon[0])
        # Create standin locatorList entries for tangency points
        if openCrv:
            if not i:
                # Get start tangency point position
                pos = cmds.pointPosition(curveShape + '.cv[1]')
                coord = closestPoint(curve, pos)
                # Create point on curve deformer
                curveCon = cmds.pointCurveConstraint(curveShape + '.u[' + str(coord) + ']', ch=1, rpo=1, w=1)
                locatorList.append(curveCon[0])
            if i == (numPt - 2):
                # Get end tangency point position
                pos = cmds.pointPosition(curveShape + '.cv[' + str(numPt) + ']')
                coord = closestPoint(curve, pos)
                # Create point on curve deformer
                curveCon = cmds.pointCurveConstraint(curveShape + '.u[' + str(coord) + ']', ch=1, rpo=1, w=1)
                locatorList.append(curveCon[0])
    ...
    for i in range(len(locatorList)):
        ...
        localOffset = cmds.getAttr(locatorList[i] + '.localPosition')[0]
        cmds.setAttr(locatorList[i] + '.translate', localOffset[0], localOffset[1], localOffset[2])
        cmds.setAttr(locatorList[i] + '.localPosition', 0, 0, 0)
        cmds.setAttr(locatorList[i] + '.localScale', locatorScale, locatorScale, locatorScale)
        # Rename Locator
        ind = str(i + 1)
        if i < 9: ind = '0' + ind
        locatorList[i] = cmds.rename(locatorList[i], prefix + 'ep' + ind + '_locator')
...
def curveToLocators(curve, locatorList, controlPoints=[]):
    ...
    # Check Locator List
    for i in range(len(locatorList)):
        if not cmds.objExists(locatorList[i]):
    ...
    for i in range(len(controlPoints)):
        cmds.connectAttr(locatorList[i] + '.worldPosition', curve + '.controlPoints[' + str(controlPoints[i]) + ']',
                       f=True)
...
def projectToSurface(curve, targetSurface, keepOriginal=False, prefix=''):
    ...
    if keepOriginal:
        ...
        cmds.delete(cmds.listRelatives(curve, c=True, type=['transform'], pa=True))
    ...
    for loc in curveLocList:
        geocmdsonstraint = cmds.geometryConstraint(targetSurface, loc, n=prefix + '_geometryConstraint')
...
def snapToCurve(curve, obj, uValue=0.0, useClosestPoint=False, snapPivot=False):
    ...
    pnt = cmds.pointPosition(curve + '.u[' + str(uValue) + ']')

    # Snap to Curve
    piv = cmds.xform(obj, q=True, ws=True, rp=True)
    if snapPivot:
        cmds.xform(obj, piv=pnt, ws=True)
    else:
        if cmds.objectType(obj) == 'transform':
            cmds.move(pnt[0] - piv[0], pnt[1] - piv[1], pnt[2] - piv[2], obj, r=True, ws=True)
        else:
            cmds.move(pnt[0], pnt[1], pnt[2], obj, ws=True, a=True)
...
def orientToCurve(curve, obj, uValue=0.0, useClosestPoint=False, upVector=(0, 1, 0), tangentAxis='x', upAxis='y'):
    ...
    # Check object
    if not cmds.objExists(obj): raise Exception('Object ' + obj + ' does not exist!!')
    rotateOrder = cmds.getAttr(obj + '.ro')

    # Get tangent at curve point
    tan = cmds.pointOnCurve(curve, pr=uValue, nt=True)
...
def sampleParam(curve, samples, useDistance=False, minPercent=0.0, maxPercent=1.0, spacing=1.0):
    ...
    # Check percent
    if minPercent < 0.0 or minPercent > 1.0: raise Exception(
        'Min percent argument is not within the valid range (0->1)!!')
    if maxPercent < 0.0 or maxPercent > 1.0: raise Exception(
        'Max percent argument is not within the valid range (0->1)!!')
    # Get percent range
...
def blendCurve(curve1, curve2, blend=0.5, keepHistory=False, prefix=''):
    ...
    avgCrv = cmds.createNode('avgCurves', n=prefix + '_avgCurves')
    # Connect curves
    cmds.connectAttr(curve1 + '.worldSpace[0]', avgCrv + '.inputCurve1', f=True)
    cmds.connectAttr(curve2 + '.worldSpace[0]', avgCrv + '.inputCurve2', f=True)
    cmds.setAttr(avgCrv + '.weight1', 1.0 - blend)
    cmds.setAttr(avgCrv + '.weight2', blend)
    cmds.setAttr(avgCrv + '.automaticWeight', 0)
    # Create target curve
    crv = prefix + '_curve'
    crvShape = cmds.createNode('nurbsCurve', n=crv + 'Shape')
    crvParent = cmds.listRelatives(crvShape, p=True)[0]
    crv = cmds.rename(crvParent, crv)
    # Connect output curve
    cmds.connectAttr(avgCrv + '.outputCurve', crv + '.create', f=True)
    # Add blend control
    if keepHistory:
        # Create blend attribute
        if not cmds.objExists(crv + '.blend'):
            cmds.addAttr(crv, ln='blend', min=0.0, max=1.0, dv=0.5, k=True)
        cmds.connectAttr(crv + '.blend', avgCrv + '.weight2', f=True)
        # Create reverse node
        rvs = cmds.createNode('reverse', n=prefix + '_reverse')
        # Connect to average curve node
        cmds.connectAttr(crv + '.blend', rvs + '.inputX', f=True)
        cmds.connectAttr(rvs + '.outputX', avgCrv + '.weight1', f=True)
    else:
        # Delete History
...
def edgeLoopCrv(meshEdgeList, rebuild=False, rebuildSpans=0, form=2, keepHistory=True, prefix=''):
    ...
    if rebuild and rebuildSpans:
        curve = \
        cmds.rebuildCurve(curve, ch=keepHistory, rpo=1, rt=0, end=1, kr=0, kcp=1, kep=1, kt=1, s=rebuildSpans, d=3,
                        tol=0.01)[0]
    # Rename curve
    curve = cmds.rename(curve, prefix + '_curve')

    # Return result
    return curve


def tubeFrocmdsurve(curve, spans=6, radius=1, upVector=(0, 1, 0), prefix=''):
    """
    ...
    # Get curve info
    crvMin = cmds.getAttr(curve + '.minValue')
    crvMax = cmds.getAttr(curve + '.maxValue')
    crvSpan = cmds.getAttr(curve + '.spans')
    ...
    for i in range(crvSpan + 2):
        ...
        crvCirc = cmds.circle(r=radius, s=spans, degree=3, n=crvPrefix + '_curve')
        crvGrp = cmds.group(crvCirc[0], n=crvPrefix + '_group')
    ...
    surface = cmds.loft(crvList, ch=1, u=1, c=0, ar=1, d=3, ss=1, rn=0, po=0, rsn=True, n=prefix + '_surface')[0]
...
def buildCmd(curve, round=True):
    ...
    for i in range(points.length()):
        if i: cmd += ','
        if round:
            cmd += '(' + ('%.3f' % points[i][0]) + ',' + ('%.3f' % points[i][1]) + ',' + ('%.3f' % points[i][2]) + ')'
        else:
            cmd += '(' + str(points[i][0]) + ',' + str(points[i][1]) + ',' + str(points[i][2]) + ')'
    cmd += '],'
...
def mirrorCurve(srcCrv, dstCrv, axis='x', space='world'):
    ...
    if srcCrvFn.nucmdsVs() != dstCrvFn.nucmdsVs():
    ...
    if space == 'world':
        ...
        raise Exception('Invalid Space "' + space + '"!')
...
def uniformRebuild(curve, spans=6, replaceOriginal=False, prefix=None):
    ...
    if not cmds.pluginInfo('glCurveUtils', q=True, l=True):
        try:
            ...
            raise MissingPluginError('Unable to load glCurveUtils!')
    ...
    uniformRebuildNode = cmds.createNode('uniformRebuild', n=prefix + '_uniformRebuild')
    cmds.setAttr(uniformRebuildNode + '.spans', spans)

    # Duplicate Curve
    rebuildCurve = cmds.duplicate(curve, rr=True, rc=True, n=prefix + '_rebuild_curve')[0]
    rebuildCurveShape = cmds.listRelatives(rebuildCurve, ni=True, s=True)[0]

    # Connect to UniformRebuild
    cmds.connectAttr(curveShape + '.local', uniformRebuildNode + '.inputCurve', f=True)
    cmds.connectAttr(uniformRebuildNode + '.outputCurve', rebuildCurveShape + '.create')

    # Replace original
    if replaceOriginal:
        rebuildCurveShape = cmds.parent(rebuildCurveShape, curve, s=True, r=True)[0]
        cmds.setAttr(curveShape + '.intermediateObject', 1)
        cmds.rename(curveShape, curveShape + 'Orig')
        cmds.rename(rebuildCurveShape, curveShape)
        cmds.delete(rebuildCurve)
        rebuildCurve = curve
...
def moveSeam(crv, numSpansToMove):
    ...
    for u in range(ncvs):
        cv = cmds.xform(crv + '.cv[' + str(u) + ']', q=True, ws=True, t=True)
    ...
    for u in range(ncvs):
        ...
        cmds.xform(crv + '.cv[' + str(u) + ']', ws=True, t=cv)
...
def avgCurves(crv1, crv2, wt1=1.0, wt2=1.0, prefix=None):
    ...
    # Create AvgCurve
    avgCrvShape = cmds.createNode('nurbsCurve')
    avgCrv = cmds.listRelatives(avgCrvShape, p=True)[0]
    avgCrv = cmds.rename(avgCrv, prefix + '_crv')

    # Create AvgCurves Node
    avgCrvNode = cmds.createNode('avgCurves', n=prefix + '_avgCurves')
    cmds.connectAttr(crv1 + '.worldSpace[0]', avgCrvNode + '.inputCurve1', f=True)
    cmds.connectAttr(crv2 + '.worldSpace[0]', avgCrvNode + '.inputCurve2', f=True)
    cmds.setAttr(avgCrvNode + '.weight1', wt1)
    cmds.setAttr(avgCrvNode + '.weight2', wt2)
    cmds.connectAttr(avgCrvNode + '.outputCurve', avgCrvShape + '.create', f=True)
...
def split(crv, param, deleteHistory=True):
    ...
    # Split Curve
    detach = cmds.detachCurve(crv, p=param, ch=True, replaceOriginal=True)
    detachCrv = cmds.ls(detach, transforms=True)
    detachCrv.insert(0, detachCrv.pop(-1))
    for i in range(len(detachCrv)):
        detachCrv[i] = cmds.rename(detachCrv[i], crv + '_split' + str(i + 1))

    # Delete History
    if deleteHistory: cmds.delete(detachCrv, ch=True)
