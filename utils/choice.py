...
def create(inputType='string', selectInput=None, inputVal=None, inputSrc=None, dst=None, prefix=None):
    ...
    # Selector Input
    if selectInput:
        if not cmds.objExists(selectInput):
    ...
    choiceNode = cmds.createNode('choice', n=prefix + ('_' * int(bool(prefix))) + 'choice')
    ...
    if selectInput:
        try:
            cmds.connectAttr(selectInput, choiceNode + '.selector', f=True)
        except Exception, e:
            raise Exception(
                'Error connecting selector input "' + selectInput + '" >> "' + choiceNode + '.selector"! Exception msg: ' + str(
                    e))
        else:
    ...
    if inputLen:
        ...
        if inputType == 'string':
            cmds.addAttr(choiceNode, ln='inputValue', dt=inputType, m=True)
        elif inputType == 'long' or inputType == 'float':
            cmds.addAttr(choiceNode, ln='inputValue', at=inputType, m=True)
        else:
        ...
        if inputVal:
            for i in range(len(inputVal)):
                if not inputVal[i] == None:
                    if inputType == 'string':
                        try:
                            cmds.setAttr(choiceNode + '.inputValue[' + str(i) + ']', inputVal[i], type=inputType)
                        except Exception, e:
                            raise Exception('Error setting input value "' + choiceNode + '.inputValue[' + str(
                                i) + ']"! Exception msg: ' + str(e))
                    elif inputType == 'long' or inputType == 'float':
                        try:
                            cmds.setAttr(choiceNode + '.inputValue[' + str(i) + ']', inputVal[i])
                        except Exception, e:
                            raise Exception('Error setting input value "' + choiceNode + '.inputValue[' + str(
                                i) + ']"! Exception msg: ' + str(e))
                    else:
        ...
        if inputSrc:
            for i in range(len(inputSrc)):
                if not inputSrc[i] == None:
                    try:
                        cmds.connectAttr(inputSrc[i], choiceNode + '.inputValue[' + str(i) + ']', f=True)
                    except Exception, e:
                        raise Exception('Error connecting input source "' + inputSrc[
                            i] + '" >> "' + choiceNode + '.inputValue[' + str(i) + ']"! Exception msg: ' + str(e))

        # Connect Input
        for i in range(inputLen):
            try:
                cmds.connectAttr(choiceNode + '.inputValue[' + str(i) + ']', choiceNode + '.input[' + str(i) + ']',
                               f=True)
            except Exception, e:
                raise Exception('Error connecting input value "' + choiceNode + '.inputValue[' + str(
                    i) + ']" >> "' + choiceNode + '.input[' + str(i) + ']"! Exception msg: ' + str(e))

        # Connect Destination
        if dst:
            try:
                cmds.connectAttr(choiceNode + '.output', dst, f=True)
            except Exception, e:
                raise Exception('Error connecting to choice destination "' + dst + '"! Exception msg: ' + str(e))
            else:
                print('Connected "' + choiceNode + '.output" >> "' + dst + '"')
