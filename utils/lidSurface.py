...
def lidSurface_curveLocators(curveList, Uvalues=[0.0, 0.5, 1.0], locatorSize=0.02):
    for curve in curveList:
        for uVal in Uvalues:
            lLocatorAndNode = cmds.pointCurveConstraint("%s.u[%s]" % (curve, uVal), constructionHistory=1,
                                                      replaceOriginal=1, weight=1)
            loc = lLocatorAndNode[0]
            cmds.xform(loc, centerPivots=1)
            locShape = cmds.listRelatives(loc, shapes=1)[0]
            for axis in ["X", "Y", "Z"]:
                cmds.setAttr(locShape + ".localScale" + axis, locatorSize)
