...
def attachToCurve(curve,
    ...
    # Check object
    if not cmds.objExists(transform): raise Exception('Object ' + transform + ' does not exist!!')
    if not glTools.utils.transform.isTransform(transform): raise Exception(
        'Object ' + transform + ' is not a valid transform!!')
    ...
    if upObject:
        if not cmds.objExists(upObject): raise Exception('WorldUp object ' + upObject + ' does not exist!!')
        if not glTools.utils.transform.isTransform(upObject): raise Exception(
            'WorldUp object ' + upObject + ' is not a valid transform!!')
    ...
    if useClosestPoint:
        uPos = cmds.xform(transform, q=True, ws=True, rp=True)
        uValue = glTools.utils.curve.closestPoint(curve, uPos)

    # Add U parameter attribute
    if not cmds.objExists(transform + '.' + uAttr):
        cmds.addAttr(transform, ln=uAttr, at='float', min=minu, max=maxu, dv=uValue, k=True)
    else:
        try:
            cmds.setAttr(transform + '.' + uAttr, uValue)
        except:
            print(
            'Unable to set existing attr "' + transform + '.' + uAttr + '" to specified value (' + str(uValue) + ')!')
    ...
    cmds.connectAttr(transform + '.' + uAttr, poc + '.parameter', f=True)

    # Create pointConstraint node
    pntCon = cmds.createNode('pointConstraint', n=prefix + '_pointConstraint')
    ...
    cmds.connectAttr(transform + '.rotatePivot', pntCon + '.constraintRotatePivot', f=True)
    cmds.connectAttr(transform + '.rotatePivotTranslate', pntCon + '.constraintRotateTranslate', f=True)

    # Attach to constrained transform
    cmds.connectAttr(pntCon + '.constraintTranslateX', transform + '.tx', f=True)
    cmds.connectAttr(pntCon + '.constraintTranslateY', transform + '.ty', f=True)
    cmds.connectAttr(pntCon + '.constraintTranslateZ', transform + '.tz', f=True)
    ...
    if orient:
        ...
        # Create aicmdsonstraint node
        aicmdson = prefix + 'aicmdsonstraint'
        aicmdson = cmds.createNode('aicmdsonstraint', n=aicmdson)

        # Attach aicmdsonstraint node
        cmds.connectAttr(poc + '.tangent', aicmdson + '.target[0].targetTranslate', f=True)
        cmds.setAttr(aicmdson + '.aimVector', *axisDict[tangentAxis])
        cmds.setAttr(aicmdson + '.upVector', *axisDict[upAxis])
        cmds.setAttr(aicmdson + '.worldUpVector', *upVector)
        cmds.setAttr(aicmdson + '.worldUpType', upTypeDict[upType])
        if upObject: cmds.connectAttr(upObject + '.worldMatrix[0]', aicmdson + '.worldUpMatrix', f=True)

        # Attach to constrained transform
        cmds.connectAttr(aicmdson + '.constraintRotateX', transform + '.rx', f=True)
        cmds.connectAttr(aicmdson + '.constraintRotateY', transform + '.ry', f=True)
        cmds.connectAttr(aicmdson + '.constraintRotateZ', transform + '.rz', f=True)
...
def attachToMesh(mesh,
    ...
    edge1_cme = cmds.createNode('curveFromMeshEdge', n=edge1_cme)
    cmds.connectAttr(mesh + '.worldMesh[0]', edge1_cme + '.inputMesh', f=True)
    cmds.setAttr(edge1_cme + '.edgeIndex[0]', edge1)
    edge2_cme = prefix + '_edge02_curveFromMeshEdge'
    edge2_cme = cmds.createNode('curveFromMeshEdge', n=edge2_cme)
    cmds.connectAttr(mesh + '.worldMesh[0]', edge2_cme + '.inputMesh', f=True)
    cmds.setAttr(edge2_cme + '.edgeIndex[0]', edge2)
    mesh_lft = prefix + '_loft'
    mesh_lft = cmds.createNode('loft', n=mesh_lft)
    cmds.connectAttr(edge1_cme + '.outputCurve', mesh_lft + '.inputCurve[0]', f=True)
    cmds.connectAttr(edge2_cme + '.outputCurve', mesh_lft + '.inputCurve[1]', f=True)
    cmds.setAttr(mesh_lft + '.degree', 1)

    # Create temporary surface
    tempSrf = cmds.createNode('nurbsSurface')
    cmds.connectAttr(mesh_lft + '.outputSurface', tempSrf + '.create')
    tempSrf = cmds.listRelatives(tempSrf, p=True)[0]
    # Attach to lofted surface
    surfAttach = attachToSurface(surface=tempSrf, transform=transform, useClosestPoint=True, orient=orient,
                                 uAxis=tangentAxis, vAxis=normAxis, prefix=prefix)
    # Set aim constraint offset to orient to normal
    offsetVal = -90.0
    if len(tangentAxis) == 2: offsetVal = 90.0
    offsetAx = str.upper(tangentAxis[-1])
    cmds.setAttr(surfAttach[2] + '.offset' + offsetAx, offsetVal)

    # Bypass temporary nurbsSurface node
    cmds.connectAttr(mesh_lft + '.outputSurface', surfAttach[0] + '.inputSurface', f=True)
...
def attachToSurface(surface,
    ...
    if cmds.objectType(surface) == 'transform':
        surfaceShape = cmds.listRelatives(surface, s=1, ni=1)[0]
    else:
        surfaceShape = surface
        surface = cmds.listRelatives(surface, p=1)[0]
    ...
    if not useClosestPoint:
        if uValue < minu or uValue > maxu: raise Exception(
            'U paramater ' + str(uValue) + ' is not within the parameter range for ' + surface + '!!')
        if vValue < minv or vValue > maxv: raise Exception(
            'V paramater ' + str(vValue) + ' is not within the parameter range for ' + surface + '!!')
    ...
    # Check closest point
    if useClosestPoint:
        uv = glTools.utils.surface.closestPoint(surface, cmds.xform(transform, q=True, ws=True, rp=True))
        uValue = uv[0]
        vValue = uv[1]
    ...
    if not cmds.objExists(transform + '.' + uAttr):
        cmds.addAttr(transform, ln=uAttr, at='float', min=minu, max=maxu, dv=uValue, k=True)
    if not cmds.objExists(transform + '.' + vAttr):
        cmds.addAttr(transform, ln=vAttr, at='float', min=minv, max=maxv, dv=vValue, k=True)

    # Create pointOnSurfaceInfo node
    pos = cmds.createNode('pointOnSurfaceInfo', n=prefix + '_pointOnSurfaceInfo')
    # Attach pointOnSurfaceInfo node
    cmds.connectAttr(surface + '.worldSpace[0]', pos + '.inputSurface', f=True)
    cmds.connectAttr(transform + '.' + uAttr, pos + '.parameterU', f=True)
    cmds.connectAttr(transform + '.' + vAttr, pos + '.parameterV', f=True)

    # Create pointConstraint node
    pntCon = cmds.createNode('pointConstraint', n=prefix + '_pointConstraint')
    # Attach pointConstraint node
    cmds.connectAttr(pos + '.position', pntCon + '.target[0].targetTranslate', f=True)
    cmds.connectAttr(transform + '.parentInverseMatrix[0]', pntCon + '.constraintParentInverseMatrix', f=True)
    cmds.connectAttr(transform + '.rotatePivot', pntCon + '.constraintRotatePivot', f=True)
    cmds.connectAttr(transform + '.rotatePivotTranslate', pntCon + '.constraintRotateTranslate', f=True)
    cmds.connectAttr(pntCon + '.constraintTranslateX', transform + '.tx', f=True)
    ...
    if orient:
        ...
        vAx = axis[vAxis]
        # Create aicmdsonstraint node
        aicmdson = cmds.createNode('aicmdsonstraint', n=prefix + '_aicmdsonstraint')
        # Attach aicmdsonstraint node
        if alignTo == 'u':
            cmds.connectAttr(pos + '.tangentU', aicmdson + '.target[0].targetTranslate', f=True)
            cmds.connectAttr(pos + '.tangentV', aicmdson + '.worldUpVector', f=True)
            cmds.setAttr(aicmdson + '.aimVector', uAx[0], uAx[1], uAx[2])
            cmds.setAttr(aicmdson + '.upVector', vAx[0], vAx[1], vAx[2])
        else:
            cmds.connectAttr(pos + '.tangentV', aicmdson + '.target[0].targetTranslate', f=True)
            cmds.connectAttr(pos + '.tangentU', aicmdson + '.worldUpVector', f=True)
            cmds.setAttr(aicmdson + '.aimVector', vAx[0], vAx[1], vAx[2])
            cmds.setAttr(aicmdson + '.upVector', uAx[0], uAx[1], uAx[2])
        cmds.connectAttr(aicmdson + '.constraintRotateX', transform + '.rx', f=True)
        cmds.connectAttr(aicmdson + '.constraintRotateY', transform + '.ry', f=True)
        cmds.connectAttr(aicmdson + '.constraintRotateZ', transform + '.rz', f=True)
        cmds.parent(aicmdson, transform)
