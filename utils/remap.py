...
class Remap(object):
    ...
    def __init__(self,
        ...
        if cmds.objExists('%s_%s' % (remapName, self._REMAPSUFFIX)):
            # Use Existing Node
    ...
    def create(self, name):
        ...
        @type name: str
        """
        self._name = cmds.createNode('remapValue', name='%s_%s' % (name, self._REMAPSUFFIX))
        return self._name
    ...
    def setAttribute(self, attr, value):
        ...
        if isinstance(value, (types.IntType, types.FloatType)):
            ...
            try:
                ...
                raise Exception('Error setting remapValue attribute "' + attr + '" value!')
            return
        ...
        # Check String Input
            ...
            if glTools.utils.attribute.isAttr(value):
                if not cmds.isConnected(value, attr):
                    try:
                        cmds.connectAttr(value, attr, f=True)
                    except:
                        raise Exception('Error connecting remapValue attribute ("' + value + '" >> "' + attr + '")!')
                    return
                else:
                    print(
                    'RemapValue node attribute "' + attr + '" already connected to source plug "' + inputValue + '"! Skipping...')
                    return
            else:
        ...
        # Invlaid Type
        raise Exception(
            'Invalid value type specified for remapValue attribute "' + attr + '"! (' + str(type(value)) + ')!')
    ...
    def connectInput(self, objectAttrName):
        """
        """
        if not cmds.isConnected(objectAttrName, '%s.inputValue' % self._name):
            cmds.connectAttr(objectAttrName, '%s.inputValue' % self._name, force=True)
    ...
    def connectOutput(self, dstAttr):
        ...
        if not glTools.utils.attribute.isAttr(dstAttr):
            raise Exception(
                'Destination attribute "' + dstAttr + '" is not a valid attribute! Unable to establish output connection...')

        # Connect Output
        outAttr = self._name + '.outValue'
        if not cmds.isConnected(outAttr, dstAttr):
            try:
                cmds.connectAttr(outAttr, dstAttr, f=True)
            except:
                raise Exception('Error connecting remapValue output ("' + outAttr + '" >> "' + dstAttr + '")!')
        else:
            print(
            'RemapValue node output "' + outAttr + '" already connected to destination plug "' + dstAttr + '"! Skipping...')
