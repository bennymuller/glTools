...
def create(baseGeo, targetGeo=[], origin='local', deformOrder=None, prefix=None):
    ...
    if deformOrder == 'after':
        blendShape = cmds.blendShape(baseGeo, name=blendShape, origin=origin, after=True)[0]
    elif deformOrder == 'before':
        blendShape = cmds.blendShape(baseGeo, name=blendShape, origin=origin, before=True)[0]
    elif deformOrder == 'parallel':
        blendShape = cmds.blendShape(baseGeo, name=blendShape, origin=origin, parallel=True)[0]
    elif deformOrder == 'split':
        blendShape = cmds.blendShape(baseGeo, name=blendShape, origin=origin, split=True)[0]
    elif deformOrder == 'foc':
        blendShape = cmds.blendShape(baseGeo, name=blendShape, origin=origin, foc=True)[0]
    else:
        blendShape = cmds.blendShape(baseGeo, name=blendShape, origin=origin)[0]
...
def addTarget(blendShape, target, base='', targetIndex=-1, targetAlias='', targetWeight=0.0, topologyCheck=False):
    ...
    if targetAlias:
        ...
        cmds.aliasAttr(targetAlias, blendShape + '.weight[' + str(targetIndex) + ']')
    ...
    if targetWeight: cmds.setAttr(blendShape + '.' + targetName, targetWeight)
...
def getTargetWeights(blendShape, target, geometry=''):
    ...
    # Get weights
    wt = cmds.getAttr(blendShape + '.it[' + str(geomIndex) + '].itg[' + str(targetIndex) + '].tw')[0]
...
def connectToTarget(blendShape, targetGeo, targetName, baseGeo, weight=1.0, force=False):
    ...
    if force:
        ...
        geomShape = cmds.listRelatives(targetGeo, s=True, ni=True)
        if geomShape:
        ...
        if hasTarget(blendShape, targetName) and weight != 1.0:
            ...
            cmds.blendShape(blendShape, e=True, ib=True, t=[baseGeo, targetIndex, targetGeo, weight])
        else:
            # Connect geometry to target input
            cmds.blendShape(blendShape, e=True, t=[baseGeo, targetIndex, targetGeo, weight])
