...
def getConnectedHairSystem(hair):
    ...
    """
    # Check object exists
    if not cmds.objExists(hair): raise Exception('Object "' + hair + '" does not exist!')
    ...
    if isFollicle(follicle):
        hairSystem = cmds.listConnections(hair + '.outHair', s=True, d=False, type='hairSystem')
...
def getConnectedFollicle(hair):
    ...
    if isHairSystem(hair):
        follicles = cmds.listConnections(hair, s=True, d=False, type='follicle')

    # Check curve
    if glTools.utils.curve.isCurve(hair):
        follicles = cmds.listConnections(hair, s=True, d=True, type='follicle')
...
def createHairSystem(name=''):
    ...
    cmds.connectAttr('time1.outTime', hairSystemShape + '.currentTime', f=True)
    hairSystem = cmds.listRelatives(hairSystemShape, p=True, pa=True)[0]
    hairSystem = cmds.rename(hairSystem, name)
...
def createFollicle(surface, u, v, prefix=''):
    ...
    if glTools.utils.surface.isSurface(surface):
        ...
        raise Exception('Invalid surface "' + surface + '"!')

    # Create follicle
    follicleShape = cmds.createNode('follicle')
    follicle = cmds.listRelatives(follicleShape, p=True, pa=True)[0]
    follicle = cmds.rename(follicle, prefix + '_follicle')

    # Connect to surface
    cmds.connectAttr(surface + '.worldMatrix[0]', follicle + '.inputWorldMatrix', f=True)
    if surfaceType == 'nurbsSurface':
        cmds.connectAttr(surface + '.local', follicle + '.inputSurface', f=True)
    elif surfaceType == 'mesh':
        cmds.connectAttr(surface + '.outMesh', follicle + '.inputMesh', f=True)
    ...
    cmds.connectAttr(follicle + '.outTranslate', follicle + '.translate', f=True)
    cmds.connectAttr(follicle + '.outRotate', follicle + '.rotate', f=True)
...
def createFollicles(ptList, childList, follicleSurface, prefix):
    ...
    for i in range(len(ptList)):
        ...
        # Parent to follicle
        if childList: cmds.parent(childList[i], follicle)

    # Return Result
    return follicleList


def makeDynamic(curveList, hairSystem='', connectRestPosition=True, follicleDegree=0, autoParentFollicle=False,
                prefix=''):
    """
    ...
    mel.eval('int $ind[1] = {0};')

    # Create hierarchy
    hairGrp = cmds.group(hairSystem, n=prefix + 'grp')
    hSysGrp = cmds.createNode('transform', n=prefix + 'follicle_grp', p=hairGrp)
    hOutGrp = cmds.createNode('transform', n=prefix + 'dynCurve_grp', p=hairGrp)
    ...
    for curve in curveList:
        ...
        if not cmds.objectType(curve) == 'nurbsCurve':
            curveShape = cmds.listRelatives(curve, s=True, ni=True, type='nurbsCurve')
        ...
        follicle = mel.eval(
            'createHairCurveNode("' + hairSystem + '","' + surface + '",' + str(u) + ',' + str(v) + ',' + str(
                nucmdsVs) + ',true,true,false,false,"' + curveShape + '",' + str(
                length) + ', $ind,"' + hSysGrp + '","' + hOutGrp + '",' + str(dynamic) + ')')
        follicle = cmds.rename(follicle, prefix + curve + '_follicle')

        # Connect Rest Position
        if connectRestPosition:
            cmds.connectAttr(curve + '.worldSpace[0]', follicle + '.restPosition', f=True)
            cmds.setAttr(follicle + '.restPose', 3)

        # Follicle Degree
        if follicleDegree: cmds.setAttr(follicle + '.degree', follicleDegree)

        # Parent Follicle
        if autoParentFollicle: follicle = cmds.parent(follicle, hSysGrp)[0]

        # Rename dynamic curve
        dynCurve = cmds.listConnections(follicle + '.outCurve', s=False, d=True)[0]
        dynCurve = cmds.rename(dynCurve, prefix + curve + '_dynCurve')
