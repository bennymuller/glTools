...
def isClip(clip):
    ...
    """
    # Check Object Exists
    if not cmds.objExists(clip): return False
    # Check Object Type
    if not cmds.objectType(clip) == 'anicmdslip': return False
    # Return Result
    return True
...
def exportClip(clip, clipPath, force=False):
    ...
    mel.eval('source "' + scriptsPath + '/doExportClipArgList.mel"')
    ...
    cmds.select(clip)
    result = mel.eval('clipEditorExportClip("' + clipPath + '", "' + ext + '")')
...
def importClip(clipPath, toChar='', toTrax=False):
    ...
    if toChar:
        ...
        mel.eval('global string $gImportToCharacter = "' + toChar + '";')
    if toTrax:
        mel.eval('global int $gScheduleClipOnCharacter = 1;')

    # Import Clip
    result = mel.eval('clipEditorImportClip("' + clipPath + '", "' + ext + '")')
...
def importClips(charSet='', clips=[], toTrax=False):
    ...
    # Get Clips
    if not clips:
        clips = cmds.fileDialog2(dir=assetPath, fileFilter='Maya Files (*.ma *.mb)', dialogStyle=2, fileMode=4,
                               okCaption='Import', caption='Load Clip')
...
def importClipsForAllChars(clips=[], toTrax=False):
    ...
    if not clips:
        ...
        # Select Clips
        clipList = cmds.fileDialog2(dir=assetPath, fileFilter='Maya Files (*.ma *.mb)', dialogStyle=2, fileMode=4,
                                  okCaption='Import', caption='Load Clip')
        clips = [os.path.splitext(os.path.basename(clip))[0] for clip in clipList]

    # Import Clips
    for charSet in cmds.ls(type='character'):
