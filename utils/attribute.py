...
def isAttr(attr):
    ...
    @type attr: str
    """
    if not cmds.objExists(attr): return False
    try:
        cmds.getAttr(attr, l=True)
    except:
        return False
    else:
        return True
...
def dataType(attr):
    ...
    dataType = cmds.getAttr(attr, type=True)
    # Return Result
    return dataType
...
def getAttrMPlug(attr):
    ...
    for i in range(2, len(attrElemList)):
        ...
        childIndex = -1
        for n in range(attrMPlug.nucmdshildren()):
...
def nextAvailableMultiIndex(attr, start=0, useConnectedOnly=True, maxIndex=10000000):
    ...
    if useConnectedOnly:
        ...
        for i in range(start, maxIndex):
            ...
            # Check connections
            conn = cmds.connectionInfo(attr + '[' + str(i) + ']', sourceFromDestination=True)
            if not conn:
                nextIndex = i
                break
    else:
        ...
        if indexCount:
            nextIndex = list(exIndexList)[-1] + 1
        else:
            nextIndex = 0
...
def distributeAttrValue(targetList, targetAttr, rangeStart=0.0, rangeEnd=1.0, smoothStep=0.0):
    ...
    for i in range(len(targetList)):
        if not cmds.objExists(targetList[i]):
            raise Exception('Object "' + targetList[i] + '" does not exist!')
        if not cmds.objExists(targetList[i] + '.' + targetAttr):
    ...
    for i in range(len(targetList)):
        ...
        cmds.setAttr(targetList[i] + '.' + targetAttr, val)
...
def deleteUserAttrs(obj, attrList=[], keepIfConnected=False):
    ...
    # Get attribute list
    if not attrList: attrList = cmds.listAttr(obj, ud=True)
    ...
    for attr in attrList:
        ...
        if cmds.objExists(obj + '.' + attr):
            ...
            # Check Connections
            if keepIfConnected:
                conns = cmds.listConnections(obj + '.' + attr, s=True, d=True)
                if conns: continue
...
def copyAttr(src, dst, attr):
    ...
    # Check/Skip Multi
    if cmds.attributeQuery(attr, n=src, m=True):
        print('Skipping multi attribute "' + src + '.' + attr + '"...')
        return src + '.' + attr

    # Get Attribute Details
    attrVal = cmds.getAttr(src + '.' + attr)
    attrType = cmds.addAttr(src + '.' + attr, q=True, at=True)
    defaultVal = cmds.addAttr(src + '.' + attr, q=True, dv=True)
    attrVisible = cmds.getAttr(src + '.' + attr, cb=True)
    attrKeyable = cmds.getAttr(src + '.' + attr, k=True)
    attrLocked = cmds.getAttr(src + '.' + attr, l=True)

    # Get Attribute Data Type
    dataType = attrType
    if attrType == 'typed':
        dataType = str(cmds.addAttr(src + '.' + attr, q=True, dt=True)[0])
    ...
    if not cmds.attributeQuery(attr, n=dst, ex=True):
        ...
        if attrType == 'typed':
            cmds.addAttr(dst, ln=attr, dt=dataType)
        else:
            cmds.addAttr(dst, ln=attr, at=attrType, dv=defaultVal)

    # Set Destination Attribute
    if attrType == 'typed':
        cmds.setAttr(dst + '.' + attr, attrVal, type=dataType)
    else:
...
def copyUserAttrs(src, dst, attrList=[], search='', replace='', copyConnections=False):
    ...
    if not attrList: attrList = cmds.listAttr(src, ud=True)
    ...
    for attr in attrList:
        ...
        attrType = cmds.addAttr(srcAttr, q=True, at=True)
        if attrType == 'typed':
            dataType = str(cmds.addAttr(src + '.' + attr, q=True, dt=True)[0])
            if dataType == 'string':
                attrVal = cmds.getAttr(srcAttr)
                attrVal = attrVal.replace(search, replace)
                cmds.setAttr(dstAttr, attrVal, type='string')

        # Copy connections
        if copyConnections:

            # Incoming connections
            inConnList = cmds.listConnections(srcAttr, s=True, d=False, p=True)
            if inConnList:
                for inConn in inConnList:
                    if search or replace:
                        inConn = inConn.replace(search, replace)
                    cmds.connectAttr(inConn, dstAttr, f=True)

            # Outgoing connections
            outConnList = cmds.listConnections(srcAttr, d=True, s=False, p=True)
            if outConnList:
                for outConn in outConnList:
                    if search or replace:
                        outConn = inConn.replace(search, replace)
                    cmds.connectAttr(dstAttr, outConn, f=True)
...
def copyAttrList(src, dst, connect=False, srcAsMaster=True, attrList=[]):
    ...
    if not attrList:
        ...
        attrList = cmds.channelBox(channelBox, q=True, selectedMainAttributes=True)
    if not attrList: return []
    ...
    for attr in attrList:
        ...
        if connect:
            if srcAsMaster:
                ...
                cmds.connectAttr(dstAttr, srcAttr, f=True)
...
def reorder(attr, pos='bottom'):
    ...
    keyAttrList = cmds.listAttr(obj, k=True)
    if not keyAttrList: keyAttrList = []
    cbAttrList = cmds.listAttr(obj, cb=True)
...
def rename(attr, name):
    ...
    result = cmds.aliasAttr(name, attr)
