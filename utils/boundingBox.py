...
def geometryBoundingBox(geometry, worldSpace=True, noIntermediate=True, visibleOnly=True):
    ...
    geoShapes = cmds.ls(cmds.listRelatives(geometry, ad=True, pa=True), noIntermediate=noIntermediate, geometry=True,
                      visible=visibleOnly)
...
def match(geometry, targetGeometry, worldSpace=True):
    ...
    if not cmds.objExists(targetGeometry):
    ...
    cmds.scale(targetSc[0] / sourceSc[0], targetSc[1] / sourceSc[1], targetSc[2] / sourceSc[2], geometry, pivot=sourcePt,
             relative=True)
    cmds.move(targetPt[0] - sourcePt[0], targetPt[1] - sourcePt[1], targetPt[2] - sourcePt[2], geometry, relative=True)
