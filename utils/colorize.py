...
def colorize(obj, color=None):
    ...
    if type(color) == str:
        if colorDict.has_key(color):
            cmds.setAttr(obj + '.overrideColor', colorDict[color])
        else:
            raise Exception('Color "' + color + '" is not recognised!!')
    elif type(color) == int:
...
def colourHierarchy(root):
    ...
    hier = cmds.ls(cmds.listRelatives(root, ad=True), dag=True)
    if not hier: hier = []
    hier.append(root)
