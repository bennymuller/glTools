...
def presetExists(nodeType, preset):
    ...
    """
    # Create temp node
    node = cmds.createNode(nodeType, n='temp_' + nodeType)

    # Check if preset is available
    try:
        mel.eval('applyAttrPreset ' + node + ' ' + preset + ' 1.0')
...
def apply(nodeList, preset, amount=1.0):
    ...
    for node in nodeList:
        ...
        nodeTypeList.append(cmds.objectType(node))
    ...
    # Apply preset
    for node in nodeList:
        # mel.eval('applyPresetToNode "'+node+'" "" "" "'+preset+'" '+str(amount))
        mel.eval('applyAttrPreset ' + node + ' ' + preset + ' ' + str(amount))
...
def save(node, preset, replaceExisting=False):
    ...
    if presetExists(nodeType, preset):
        if not replaceExisting:
            raise Exception(
                'Attr preset "' + preset + '" for node type "' + nodeType + '" already exists! Set replaceExisting=True to overwrite!')
        else:
            print('Overwritting existing "' + preset + '" preset for node type "' + nodeType + '"!')

    # Save preset
    mel.eval('saveAttrPreset ' + node + ' ' + preset + ' false')
