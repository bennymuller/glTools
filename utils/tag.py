...
class Tag(object):
    ...
    def addNameTag(self, control, tag):
        ...
        # Add Tag attribute
        if cmds.objExists(control + '.' + self.nameTagAttr):
            cmds.addAttr(control, ln=self.nameTagAttr, dt='string')
        cmds.setAttr(control + '.' + self.nameTagAttr, tag, type='string')
    ...
    def getNameTag(self, control):
        ...
        if not cmds.objExists(control): raise Exception('Object ' + control + ' does not exist!')
        # Check tag attribute
        if not cmds.objExists(control + '.' + self.nameTagAttr): raise Exception(
            'Object ' + control + ' does not have a "' + self.nameTagAttr + '" attribute!')
        # Return tag string value
        return cmds.getAttr(control + '.' + self.nameTagAttr)
    ...
    def guessNameTag(self, control, side=True, part=True, optSide=True, subPart=True, node=False):
        ...
        controlElecmdsnt = len(controlElem)
        controlElemInd = 0

        # Check number of elements
        if controlElecmdsnt < 3: print 'Warning: Name pattern does not match naming convention'
