...
DEFAULT_NODES = [u'characterPartition', u'defaultHardwareRenderGlobals', u'defaultLayer', u'defaultLightList1',
                 u'defaultLightSet',
                 u'defaultObjectSet', u'defaultRenderGlobals', u'defaultRenderLayer', u'defaultRenderLayerFilter',
                 u'defaultRenderQuality',
                 u'defaultRenderUtilityList1', u'defaultResolution', u'defaultShaderList1', u'defaultTextureList1',
                 u'dof1', u'dynController1',
                 u'globalCacheControl', u'hardwareRenderGlobals', u'hyperGraphInfo', u'hyperGraphLayout', u'ikSystem',
                 u'initialMaterialInfo',
                 u'initialParticleSE', u'initialShadingGroup', u'lambert1', u'layerManager', u'layersFilter',
                 u'lightLinker1', u'lightList1',
                 u'objectNameFilter4', u'objectScriptFilter10', u'objectTypeFilter69', u'objectTypeFilter70',
                 u'objectTypeFilter71',
                 u'particleCloud1', u'postProcessList1', u'relationshipPanel1LeftAttrFilter',
                 u'relationshipPanel1RightAttrFilter',
                 u'renderGlobalsList1', u'renderLayerFilter', u'renderLayerManager', u'renderPartition',
                 u'renderingSetsFilter', u'shaderGlow1',
                 u'strokeGlobals', u'test:brush1', u'time1', u'|front', u'|front|frontShape', u'|persp',
                 u'|persp|perspShape', u'|side',
                 u'|side|sideShape', u'|top', u'|top|topShape', u'front', u'frontShape', u'persp', u'perspShape',
                 u'side',
...
def getRelatedNS(ns, mode='children'):
    ...
    if not cmds.namespace(exists=":%s" % ns):
    ...
    if mode == 'children' or mode == 'both':
        ...
        while queue:
            ...
            # set the namspace
            cmds.namespace(set=':%s' % cur)
            # get children and add to queue
            children = cmds.namespaceInfo(listOnlyNamespaces=True)
            if children:
...
def createNS(namespace):
    ...
    if cmds.objExists(namespace):
        raise NamespacerError(
            'Unable to add namespace "%s" to your scene since a node exists with the same name' % namespace)
...
def pushNS(namespace=':'):
    ...
    """
    HISTORY_STACK.append(':' + cmds.namespaceInfo(cur=True))
...
def popNS():
    ...
    """
    if HISTORY_STACK:
        cmds.namespace(set=HISTORY_STACK.pop(-1))
    else:
...
def renameNS(origNamespace, newNamespace):
    ...
    if not cmds.namespace(exists=":" + origNamespace):
    ...
    if origNamespace == ':':
        setNS(cmds.ls(), newNamespace, force=True)
        return False
    else:
        # move it
        cmds.namespace(force=True, mv=[origNamespace, newNamespace])
        ...
        if stillExists:
            logger.warning(
                'The namespace "%s" was renamed to "%s" but it was not successfully deleted, some nodes may still be in origional namespace' % origNamespace, newNamespace)
...
def deleteNS(namespace):
    ...
    if exists(namespace):
        logger.warning(
            'The namespace %s was not successfully deleted (likely due to references in namespace' % namespace)
...
def stripNS(nodes, force=True, autoExpandShapes=True):
    ...
    for node in nodes:
        ...
        if cmds.objExists(baseNode) and not force:
            continue

        newName = cmds.rename(node, baseNode)
...
def setNS(nodes, namespace, force=True, autoExpandShapes=True, prefix=False):
    ...
    for node in nodes:
        ...
        # see if the non-namespaced node exists
        if cmds.objExists(newNode) and not force:
            logger.warning(
                'namespacer.setNS(): skipping renaming "%s" to "%s" because a name conflict will result, use force flag to apply change'
                % (node, newNode))
            continue
...
def searchReplaceNS(nodes, search, replace, force=True, autoExpandShapes=True):
    ...
    if search == ":":
        logger.error(
            'You are attempting to searchReplaceNS without any search string.. use appendNS for this functionality')
        return {}
...
def _processNodes(nodes, autoExpandShapes=True):
    ...
    for node in nodes:
        ...
        # handle shapes
        if autoExpandShapes:
            shapes = cmds.listRelatives(node, shapes=True, pa=True)
            if shapes:
                newNodes.update(set(shapes))

    # sort the nodes so we rename them in the right order
    nodes = sorted(cmds.ls(list(newNodes), long=True))
    nodes.reverse()
    return nodes
...
def importToNamespace(filePath, namespace, group=False, removeExistingNamespaces=False):
    ...
    @todo: could using the cmds.file(rnn) flag which returns new nodes be used to optimize this?
    ...
    cmds.file(filePath, i=True, namespace=tempNS, groupReference=group)
    ...
    diffObjects = set(cmds.ls()).difference(origObjects)

    # logger.info('importToNamespace() took %s seconds' % cmds.timerX(startTime=timer))
    return diffObjects
...
def generateTmpNamespace(base='tmpNS', create=False):
    ...
    """
    cnt = 1
    while cmds.namespace(exists=base):
        base += str(cnt)
        cnt += 1
...
def referenceToNamespace(filepath, namespace=None):
    ...
    if not namespace:
        referencedNodes = cmds.file(filepath, r=True, dns=True, rnn=True)
    elif len(namespace) > 1:
        setLiveNS(listToNamespace(namespace[:-1]))
        referencedNodes = cmds.file(filepath, r=True, namespace=namespace[-1], rnn=True)
    else:
        referencedNodes = cmds.file(filepath, r=True, namespace=namespace[0], rnn=True)

    # cleanup
    setLiveNS(curNS)

    refNodes = cmds.ls(referencedNodes, type='reference', long=True)
    if refNodes:
        return refNodes[0]
    else:
    ...
    for n in nodes:
        if cmds.objExists(n):
            try:
                ...
                logger.debug('unloadNamespace was unable to delete the namespaced node "' + str(
                    n) + '" during unload procedure..')
                problemFlag = True
...
def findAssemblies(namespace):
    ...
    if namespace != ":":
        namespace += ":"
    assemblies = cmds.ls(':%s*' % namespace, assemblies=True)
    return assemblies
