...
def isSurface(surface):
    ...
    """
    # Check object exists
    if not cmds.objExists(surface): return False
    # Check shape
    if cmds.objectType(surface) == 'transform': surface = cmds.listRelatives(surface, s=True, ni=True, pa=True)[0]
    if cmds.objectType(surface) != 'nurbsSurface': return False
...
def getSurfaceFn(surface):
    ...
    if cmds.objectType(surface) == 'transform':
        surface = cmds.listRelatives(surface, s=True, ni=True, pa=True)[0]
...
def chordLength(surface, param=0.0, direction='u'):
    ...
    curve = cmds.duplicateCurve(surface + '.' + direction + '[' + str(param) + ']', ch=0, rn=0, local=0)
...
def locatorSurface(surface, controlPoints=[], locatorScale=0.075, prefix=''):
    ...
    for cv in controlPoints:
        ...
        cmds.connectAttr(loc + '.worldPosition[0]', surface + '.controlPoints[' + str(ind) + ']')
...
def surfaceArea(surface, worldSpace=True):
    ...
    """
    # Check Surface
    if not cmds.objExists(surface): raise Exception('Object ' + surface + ' does not exist!')
    if cmds.objectType(surface) == 'transform':
        surfaceShape = cmds.listRelatives(surface, s=True, ni=True, pa=True)[0]
        if cmds.objectType(surfaceShape) != 'nurbsSurface':
    ...
    area = 0.0
    if worldSpace:
        area = surfaceFn.area(OpenMaya.MSpace.kWorld)
    else:
        area = surfaceFn.area(OpenMaya.MSpace.kObject)
...
def snapToSurface(surface, obj, uValue=0.0, vValue=0.0, useClosestPoint=False, snapPivot=False):
    ...
    if uValue < minu or uValue > maxu: raise Exception(
        'U paramater ' + str(uValue) + ' is not within the U parameter range for ' + surface + '!!')
    if vValue < minv or vValue > maxv: raise Exception(
        'V paramater ' + str(vValue) + ' is not within the V parameter range for ' + surface + '!!')

    # Get surface point position
    pnt = cmds.pointPosition(surface + '.uv[' + str(uValue) + '][' + str(vValue) + ']')
...
def orientToSurface(surface,
    ...
    if uValue < minu or uValue > maxu: raise Exception(
        'U paramater ' + str(uValue) + ' is not within the U parameter range for ' + surface + '!!')
    if vValue < minv or vValue > maxv: raise Exception(
        'V paramater ' + str(uValue) + ' is not within the V parameter range for ' + surface + '!!')
...
def rebuild_old(surface, spansU=0, spansV=0, fullRebuildU=False, fullRebuildV=False, replaceOrig=False):
    ...
    iso_crv = cmds.duplicateCurve(surface + '.u[' + str(u) + ']', ch=0, rn=0, local=0)[0]
    iso_len = cmds.arclen(iso_crv)
    ...
    iso_crv = cmds.duplicateCurve(int_surface + '.v[' + str(v) + ']', ch=0, rn=0, local=0)[0]
    iso_len = cmds.arclen(iso_crv)
...
def projectToSurface(surface, targetSurface, direction='u', keepOriginal=False, prefix=''):
    ...
    for i in range(spans + 1):
        ...
        srfCurve = cmds.duplicateCurve(surface + '.' + direction.lower() + '[' + str(i * spanInc) + ']', ch=0, rn=0,
                                     local=0, n=srfCurveName)
        ...
        for loc in curveLocatorList:
            ...
            geocmdsonstraint = cmds.geometryConstraint(targetSurface, loc, n=geocmdsonstraint)
    ...
    # Loft new surface
    surfaceLoft = cmds.loft(curveList, ch=1, u=1, c=0, ar=1, d=3, ss=1, rn=0, po=0, rsn=True)
    ...
    # Return result
    return [surface, loft, curveList, curveGrpList, curveLocList, geocmdsonstraintList]
...
def rebuildFromIsoparms(surface, spansU=0, spansV=0, degree=3, keepHistory=False):
    ...
    # Get surface form
    closeU = bool(cmds.getAttr(surface + '.formU'))
    closeV = bool(cmds.getAttr(surface + '.formV'))
    ...
    # Rebuild in U
    uLoft = cmds.loft([surface + '.u[' + str(i) + ']' for i in uList], close=closeU, degree=degree)
    uSurface = uLoft[0]

    # Rebuld in V
    vLoft = cmds.loft([uSurface + '.v[' + str(i) + ']' for i in vList], close=closeV, degree=degree)
