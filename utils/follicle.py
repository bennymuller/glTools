...
def isFollicle(follicle):
    ...
    if 'transform' in cmds.nodeType(follicle, i=True):
        follicleShape = cmds.ls(cmds.listRelatives(follicle, s=True, ni=True, pa=True) or [], type='follicle')
...
def create(targetGeo,
    ...
    if uvSet and targetIsMesh:
        if not uvSet in cmds.polyUVSet(targetGeo, q=True, auv=True):
    ...
    # Set Parameter
    cmds.setAttr(follicle + '.parameterU', parameter[0])
    cmds.setAttr(follicle + '.parameterV', parameter[1])

    # UV Set
    if uvSet: cmds.setAttr(follicle + '.mapSetName', uvSet, type='string')
    ...
    # Connect Geometry
    if targetIsMesh: cmds.connectAttr(targetGeo + '.outMesh', follicle + '.inputMesh', f=True)
    if targetIsSurface: cmds.connectAttr(targetGeo + '.localSpace[0]', follicle + '.inputSurface', f=True)

    # Connect WorldSpace
    cmds.connectAttr(targetGeo + '.worldMatrix[0]', follicle + '.inputWorldMatrix', f=True)
...
def buildAtPoint(pt,
    ...
    # UV Set
    if uvSet and targetIsMesh:
        if not uvSet in cmds.polyUVSet(geo, q=True, auv=True):
            raise Exception('Target geometry has no UV set "' + uvSet + '"!')
