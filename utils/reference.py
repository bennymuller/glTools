...
def isProxyManager(proxyManager):
    ...
    if not cmds.objExists(proxyManager): return False
    if cmds.objectType(proxyManager) != 'proxyManager': return False
...
def isLoaded(refNode):
    ...
    isLoaded = not cmds.file(referenceNode=refNode, q=True, deferReference=True)
...
def getReferenceProxyManager(refNode):
    ...
    if not cmds.attributeQuery('proxyMsg', n=refNode, ex=True):
        print('Reference "' + refNode + '" has no "proxyMsg" attribute! Unable to determine proxy manager...')
        return None

    # Get Connected Proxy Manager
    proxyManager = cmds.ls(cmds.listConnections(refNode + '.proxyMsg', s=True, d=False) or [], type='proxyManager') or []
    if not proxyManager:
...
def getReferencesFromProxyManager(proxyManager):
    ...
    refList = cmds.listConnections(proxyManager + '.proxyList', s=False, d=True) or []
...
def getNamespace(refNode):
    ...
    # ns = cmds.file(refFile,q=True,ns=True)
    ...
    for refNode in listReferences(parentNS):
        ...
        if refNS == parentNS + ns:
            ...
            if cmds.attributeQuery('proxyMsg', n=refNode, ex=True):
                proxyManager = cmds.ls(cmds.listConnections(refNode + '.proxyMsg', s=True, d=False) or [],
                                     type='proxyManager') or []
                if proxyManager:
                    # Check Active Proxy
                    activeProxyPlug = cmds.connectionInfo(proxyManager[0] + '.activeProxy', destinationFromSource=True)[0]
                    activeProxyInfo = cmds.connectionInfo(activeProxyPlug, destinationFromSource=True)
                    if not activeProxyInfo: raise Exception(
                        'Error getting active reference from proxy manager "' + proxyManager[0] + '"!')
...
def getReferencedNodes(refNode):
    ...
    nodes = cmds.referenceQuery(refNode, nodes=True, dagPath=True)
    if not nodes: return []
...
def replaceReference(refNode, refPath, verbose=True):
    ...
    # Get file type
    refType = ''
    if refPath.endswith('.ma'):
        refType = 'mayaAscii'
    elif refPath.endswith('.mb'):
        refType = 'mayaBinary'
    else:
        raise Exception('Invalid file type! ("' + refPath + '")')
...
def renameReferenceNamespace(refNS, newNS):
    ...
    """
    # Check Namespace
    if cmds.namespace(ex=newNS): raise Exception(
        'Namespace "' + newNS + '" already exists! Unable to rename reference namespace...')
    ...
    # Rename Reference Namespace
    cmds.file(refPath, e=True, namespace=newNS)
...
def recordSourcePath(targetNode):
    ...
    # Add String Attr
    cmds.addAttr(targetNode, ln='sourceReferencePath', dt='string')
    # Set String Attr Value
    cmds.setAttr(targetNode + '.sourceReferencePath', realPath, type='string')
    # Lock Attribute
    # cmds.setAttr(targetNode+'.sourceReferencePath',l=True)
...
def getEditNodes(refNode,
    ...
    if parent and setAttr and addAttr and deleteAttr and connectAttr and disconnectAttr:
        nodeList.extend(mel.eval(refQueryCmd + ' -editNodes ' + refNode) or [])
    else:
        if parent: nodeList.extend(mel.eval(refQueryCmd + ' -editCommand parent -editNodes ' + refNode) or [])
        if setAttr: nodeList.extend(mel.eval(refQueryCmd + ' -editCommand setAttr -editNodes ' + refNode) or [])
        if addAttr: nodeList.extend(mel.eval(refQueryCmd + ' -editCommand addAttr -editNodes ' + refNode) or [])
        if deleteAttr: nodeList.extend(mel.eval(refQueryCmd + ' -editCommand deleteAttr -editNodes ' + refNode) or [])
        if connectAttr: nodeList.extend(mel.eval(refQueryCmd + ' -editCommand connectAttr -editNodes ' + refNode) or [])
        if disconnectAttr: nodeList.extend(
            mel.eval(refQueryCmd + ' -editCommand disconnectAttr -editNodes ' + refNode) or [])
...
def getEditAttrs(refNode,
    ...
    attrList = []
    if parent and setAttr and addAttr and deleteAttr and connectAttr and disconnectAttr:
        attrList.extend(mel.eval(refQueryCmd + ' -editAttrs ' + node) or [])
    else:
        if parent: attrList.extend(mel.eval(refQueryCmd + ' -editCommand parent -editAttrs ' + node) or [])
        if setAttr: attrList.extend(mel.eval(refQueryCmd + ' -editCommand setAttr -editAttrs ' + node) or [])
        if addAttr: attrList.extend(mel.eval(refQueryCmd + ' -editCommand addAttr -editAttrs ' + node) or [])
        if deleteAttr: attrList.extend(mel.eval(refQueryCmd + ' -editCommand deleteAttr -editAttrs ' + node) or [])
        if connectAttr: attrList.extend(mel.eval(refQueryCmd + ' -editCommand connectAttr -editAttrs ' + node) or [])
        if disconnectAttr: attrList.extend(
            mel.eval(refQueryCmd + ' -editCommand disconnectAttr -editAttrs ' + node) or [])
...
def getEditCommands(refNode,
    ...
    cmdList = []
    if parent and setAttr and addAttr and deleteAttr and connectAttr and disconnectAttr:
        cmdList.extend(mel.eval(refQueryCmd + ' -editStrings ' + node) or [])
    else:
        if parent: cmdList.extend(mel.eval(refQueryCmd + ' -editCommand parent -editStrings ' + node) or [])
        if setAttr: cmdList.extend(mel.eval(refQueryCmd + ' -editCommand setAttr -editStrings ' + node) or [])
        if addAttr: cmdList.extend(mel.eval(refQueryCmd + ' -editCommand addAttr -editStrings ' + node) or [])
        if deleteAttr: cmdList.extend(mel.eval(refQueryCmd + ' -editCommand deleteAttr -editStrings ' + node) or [])
        if connectAttr: cmdList.extend(mel.eval(refQueryCmd + ' -editCommand connectAttr -editStrings ' + node) or [])
        if disconnectAttr: cmdList.extend(
            mel.eval(refQueryCmd + ' -editCommand disconnectAttr -editStrings ' + node) or [])
...
def removeReferenceEdits(refNode,
    ...
    if parent and setAttr and addAttr and deleteAttr and connectAttr and disconnectAttr:
        ...
        mel.eval(refQueryCmd + ' -removeEdits ' + node)
    else:
        # Remove Edits Per Command
        if parent: mel.eval(refQueryCmd + '-editCommand parent -removeEdits ' + node)
        if setAttr: mel.eval(refQueryCmd + '-editCommand setAttr -removeEdits ' + node)
        if addAttr: mel.eval(refQueryCmd + '-editCommand addAttr -removeEdits ' + node)
        if deleteAttr: mel.eval(refQueryCmd + '-editCommand deleteAttr -removeEdits ' + node)
        if connectAttr: mel.eval(refQueryCmd + '-editCommand connectAttr -removeEdits ' + node)
        if disconnectAttr: mel.eval(refQueryCmd + '-editCommand disconnectAttr -removeEdits ' + node)
...
def removeParentEdits(node=''):
    ...
    mel.eval('referenceEdit -removeEdits -failedEdits true -successfulEdits true -editCommand parent ' + refNode)
...
def removeSetAttrEdits(node='', refFile=''):
    ...
    # Remove Edits - referenceEdit command not working correctly using python WTF??
    mel.eval('referenceEdit -removeEdits -failedEdits true -successfulEdits true -editCommand setAttr "' + refFile + '"')
...
def removeConnectAttrEdits(nodeList=''):
    ...
    for node in nodeList:
        mel.eval('referenceEdit -failedEdits true -successfulEdits true -editCommand connectAttr -removeEdits ' + node)
        mel.eval(
            'referenceEdit -failedEdits true -successfulEdits true -editCommand disconnectAttr -removeEdits ' + node)
