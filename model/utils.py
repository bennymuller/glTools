...
def setVerticalPlacement(obj, freezeTransform=False):
    ...
    # Set position
    cmds.move(0, -base, 0, obj, ws=True, r=True)

    # Freeze Transforms
    if freezeTransform: cmds.makeIdentity()
...
def alignControlPoints(controlPointList, axis, localAxis=True):
    ...
    controlPointList = cmds.filterExpand(controlPointList, sm=[28, 31, 46])
    ...
    for cp in controlPointList:
        ptList.append(cmds.pointPosition(cp, w=not (localAxis), l=localAxis))
    ...
    for i in range(len(ptList)):
        print i
        cmds.move(ptList[i][0], ptList[i][1], ptList[i][2], controlPointList[i], a=True, ws=not (localAxis), os=localAxis)
...
def slideDeformer(geo):
    ...
    slideDeformer = cmds.deformer(geo, type='slideDeformer', n=prefix + '_slideDeformer')[0]
    ...
    try:
        ...
        pass
    if inputMesh: cmds.connectAttr(inputMesh + '.outMesh', slideDeformer + '.referenceMesh', f=True)
...
def strainRelaxer(geo):
 ...
 \
    ...
    strainRelaxer = cmds.deformer(geo, type='strainRelaxer', n=prefix + '_strainRelaxer')[0]
    ...
    try:
        ...
        pass
    if inputMesh: cmds.connectAttr(inputMesh + '.outMesh', strainRelaxer + '.refMesh', f=True)
...
def directionalSmooth(geo):
    ...
    directionalSmooth = cmds.deformer(geo, type='directionalSmooth', n=prefix + '_directionalSmooth')[0]
    ...
    try:
        ...
        pass
    if inputMesh: cmds.connectAttr(inputMesh + '.outMesh', directionalSmooth + '.referenceMesh', f=True)
