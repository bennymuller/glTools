...
def buildData(node):
    ...
    # Check Set
    if 'objectSet' in cmds.nodeType(node, i=True):
    ...
    raise Exception('Unsupported node type! Unable to build data for "' + node + '" (' + cmds.nodeType(node) + ')')
...
def buildConstraintData(constraint):
    ...
    if constraintType == 'aicmdsonstraint':
        cDataObj = constraintData.AicmdsonstraintData
        ...
        print(
        'Unsupported Constraint! Using base ConstraintData class for ' + constraintType + ' "' + constraint + '"!')
    ...
    cData = cDataObj()
    try:
        cData.buildData(constraint)
    except:
        raise Exception('ConstraintData: Error building data object for "' + constraint + '"!')
