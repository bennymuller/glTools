...
class DeformerData(data.Data):
    ...
    def buildData(self, deformer):
        ...
        if not glTools.utils.deformer.isDeformer(deformer):
            raise Exception(
                'Object ' + deformer + ' is not a valid deformer! Unable to instantiate DeformerData() class object!')
        ...
        for geo in self._data['affectedGeometry']:
            geoShape = cmds.listRelatives(geo, s=True, ni=True, pa=True)[0]
            self._data[geo] = {}
            self._data[geo]['index'] = affectedGeo[geo]
            self._data[geo]['geometryType'] = str(cmds.objectType(geoShape))
    ...
    def rebuild(self, overrides={}):
        ...
        if not cmds.objExists(self._data['name']):
            deformer = cmds.deformer(self.getMemberList(), typ=self._data['type'], n=deformer)[0]
        else:
    ...
    def setDeformerMembership(self, geoList=[]):
        ...
        for geo in geoList:
            ...
            if removeMembers:
                ...
                # Mesh
                if self._data[geo]['geometryType'] == 'mesh':
                    cmds.sets([geo + '.vtx[' + str(i) + ']' for i in removeMembers], rm=deformerSet)
                # Curve
                if self._data[geo]['geometryType'] == 'nurbsCurve':
                    cmds.sets([geo + '.cv[' + str(i) + ']' for i in removeMembers], rm=deformerSet)
                # Particle
                if self._data[geo]['geometryType'] == 'particle':
                    cmds.sets([geo + '.pt[' + str(i) + ']' for i in removeMembers], rm=deformerSet)
                # Surface
                if self._data[geo]['geometryType'] == 'nurbsSurface':
                    cmds.sets([geo + '.cv[' + str(i[0]) + '][' + str(i[1]) + ']' for i in removeMembers], rm=deformerSet)
                # Lattice
                if self._data[geo]['geometryType'] == 'lattice':
                    cmds.sets(
                        [geo + '.pt[' + str(i[0]) + '][' + str(i[1]) + '][' + str(i[2]) + ']' for i in removeMembers],
                        rm=deformerSet)
            ...
            cmds.sets(self.getMemberList([geo]), fe=deformerSet)
    ...
    def getDeformerAttrValues(self):
        ...
        for attr in self._data['attrValueList']:
            if not cmds.getAttr(deformer + '.' + attr, se=True) and cmds.listConnections(deformer + '.' + attr, s=True,
                                                                                     d=False):
    ...
    def getDeformerAttrConnections(self):
        ...
        for attr in self._data['attrConnectionList']:
            attrConn = cmds.listConnections(deformer + '.' + attr, s=True, d=False, p=True, sh=True,
    ...
    def setDeformerAttrValues(self):
        ...
        for attr in self._data['attrValueDict'].iterkeys():
            ...
            if type(attrVal) == list or type(attrVal) == tuple:
                cmds.setAttr(deformerAttr, *attrVal)
            else:
                cmds.setAttr(deformerAttr, attrVal)
    ...
    def setDeformerAttrConnections(self):
        ...
        for attr in self._data['attrConnectionDict'].iterkeys():
            ...
            if not cmds.objExists(deformerAttr):
                print('Deformer attribute connection destination "' + deformerAttr + '" does not exist!(' + self._data[
                    'name'] + ')')
                continue
            # Check connection destination settable state
            if not cmds.getAttr(deformerAttr, se=True):
                print('Deformer attribute connection destination "' + deformerAttr + '" is not settable!(' + self._data[
                    'name'] + ')')
                continue
            # Check connection source
            if not cmds.objExists(self._data['attrConnectionDict'][attr]):
                print('Deformer attribute connection source "' + self._data['attrConnectionDict'][
                    attr] + '" does not exist!(' + self._data['name'] + ')')
                continue
            # Create Connection
            cmds.connectAttr(self._data['attrConnectionDict'][attr], deformerAttr, f=True)
