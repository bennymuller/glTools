...
class MayaData(data.Data):
    ...
    def saveAs(self):
        ...
        """
        # Specify File Path
        filePath = cmds.fileDialog2(fileFilter=self.fileFilter, dialogStyle=2, caption='Save As')
    ...
    def load(self, filePath=''):
        ...
        # Check File Path
        if not filePath:
            filePath = cmds.fileDialog2(fileFilter=self.fileFilter, dialogStyle=2, fileMode=1, caption='Load Data File',
                                      okCaption='Load')
            if not filePath: return
            filePath = filePath[0]
        else:
