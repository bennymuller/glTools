...
class ClusterData(deformerData.DeformerData):
    ...
    def buildData(self, cluster):
        ...
        clsHandle = cmds.listConnections(cluster + '.matrix', s=True, d=False, sh=True)
        if clsHandle:
            self._data['clusterHandle'] = clsHandle[0]
    ...
    def rebuild(self, overrides={}):
        ...
        if not cmds.objExists(self._data['name']):
            # Create New Cluster
            cluster = cmds.cluster(self.getMemberList(), n=self._data['name'])
            self._data['name'] = cluster[0]
        else:
            # Check Cluster
            if cmds.objectType(self._data['name']) != 'cluster':
        ...
        if self._data['clusterHandle']:
            if not cmds.objExists(self._data['clusterHandle']):
                raise Exception('Weighted Node "' + self._data['clusterHandle'] + '" does not exist!')
            try:
                cmds.cluster(self._data['name'],
                             edit=True,
                             bindState=True,
