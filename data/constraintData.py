...
class ConstraintData(data.Data):
    ...
    def buildData(self, nodeList):
        ...
        for constraint in nodeList:
            ...
            self._data[constraint]['targetAliasList'] = [glTools.utils.constraint.targetAlias(constraint, target) for
                                                         target in targetList]
            self._data[constraint]['targetIndex'] = [glTools.utils.constraint.targetIndex(constraint, target) for target
                                                     in targetList]
            self._data[constraint]['targetWeight'] = [cmds.getAttr(constraint + '.' + target) for target in targetList]

            # Constraint Slave Attributes
            constraintAttrs = [cmds.ls(i, o=True)[0] for i in
                               cmds.listConnections(constraint, s=False, d=True, p=True, sh=True) or [] if
                               i.startswith(slave + '.')]
            self._data[constraint]['skipTranslate'] = sorted(
                [i for i in self.TRANSLATE.keys() if not self.TRANSLATE[i] in constraintAttrs])
            self._data[constraint]['skipRotate'] = sorted(
                [i for i in self.ROTATE.keys() if not self.ROTATE[i] in constraintAttrs])
        ...
        for constraint in self._data.iterkeys():
            ...
            for attr in self.ATTR_VAL:
                if cmds.attributeQuery(attr, n=constraint, ex=True):
                    ...
                    print(
                    self.name() + ': ' + self.TYPE + ' "' + constraint + '" has no attribute "' + attr + '"! Skipping...')
    ...
    def restoreAttrValues(self):
        ...
        for constraint in self._data.iterkeys():
            ...
            if not cmds.objExists(constraint):
                print(
                self.name() + ': ' + self.TYPE + ' "' + constraint + '" does not exist! Unable to restore attribute values...')
            ...
            for attr in self.ATTR_VAL:
                ...
                if self._data[constraint].has_key(attr):
                    ...
                    try:
                        ...
                        print(
                        self.name() + ': Unable to set constraint attribute value "' + constraint + '.' + attr + '"! Exception msg: ' + str(
                            e))
                else:
            ...
            for attr in self.ATTR_CON:
                ...
                if self._data[constraint].has_key(attr):
                    ...
                    if cmds.objExists(src):
                        try:
                            cmds.connectAttr(src, constraint + '.' + attr, f=force)
                        except Exception, e:
                            print(
                            self.name() + ': Unable to connect "' + src + '" to "' + constraint + '.' + attr + '"! Exception msg:' + str(
                                e))
                    else:
                        print(
                        self.name() + ': Attribute connection source "' + src + '" does not exist! Unable to restore attribute connection...')
                else:
...
class AicmdsonstraintData(ConstraintData):
    """
    """
    TYPE = 'aicmdsonstraint'
...
class ParentConstraintData(ConstraintData):
    ...
    def rebuildConstraint(self, constraint):
        ...
        for target in range(len(constraintTarget)):
            ...
            cmds.setAttr(constraint + '.target[' + str(targetIndex) + '].targetOffsetTranslate', offsetT)
            cmds.setAttr(constraint + '.target[' + str(targetIndex) + '].targetOffsetRotate', offsetR)
