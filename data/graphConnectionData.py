...
class GraphConnectionData(data.Data):
    ...
    def save(self, filePath, force=False):
        ...
        if os.path.isfile(mayaFilePath) and not force:
            raise Exception(
                'File "' + mayaFilePath + '" already exists! Use "force=True" to overwrite the existing file.')

        # Check File Path for .pkl file
        if os.path.isfile(pklFilePath) and not force:
            raise Exception(
                'File "' + pklFilePath + '" already exists! Use "force=True" to overwrite the existing file.')

        # save maya file
        cmds.select(cl=True)
        cmds.select(self.nodeList)
        cmds.file(mayaFilePath, force=force, type="mayaBinary", exportSelected=True, ch=False, channels=False)
        cmds.select(cl=True)
    ...
    def load(self, filePath=''):
        ...
        # Check File Path
        if not filePath:
            filePath = cmds.fileDialog2(fileFilter=self.fileFilter, dialogStyle=2, fileMode=1,
                                      caption='Load Graph Connection Maya File', okCaption='Load')
    ...
    def importFile(self):
        ...
        # Open File
        try:
            self.nodeList = cmds.file(self.mayaFilePath, type="mayaBinary", i=True, returnNewNodes=True)
        except:
