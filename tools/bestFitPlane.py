...
def bestFitPlaneNormal(ptList):
    ...
    for i in range(len(ptList)):
        ...
        norm += OpenMaya.MVector((prev.z + curr.z) * (prev.y - curr.y), (prev.x + curr.x) * (prev.z - curr.z),
...
def bestFitPlaneCreate(ptList, upVector=(0, 1, 0)):
    ...
    # Create Plane
    plane = cmds.polyPlane(w=1, h=1, sx=1, sy=1, ax=[0, 1, 0], cuv=2, ch=False)[0]

    # Position Plane
    cmds.rotate(rot[0], rot[1], rot[2], plane, os=True, a=True)
    cmds.move(pt[0], pt[1], pt[2], plane, ws=True, a=True)
