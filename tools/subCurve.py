...
def fromRefCurve(curve, refCurve, rebuildToRef=True):
    ...
    # SubCurve
    subCurve = cmds.detachCurve(curve, p=[stParam, enParam], replaceOriginal=False, ch=False)
    ...
    if rebuildToRef:
        ...
        cmds.rebuildCurve(subCurve, ch=0, rpo=1, rt=0, end=1, kr=0, kcp=0, kep=1, kt=0, s=spans, d=degree, tol=0)
...
def fromPoint(curve, pnt=[0.0, 0.0, 0.0], replaceOrig=False, keepHistory=True):
    ...
    # Detach curve
    subCurve = cmds.detachCurve(curve, p=param, replaceOriginal=replaceOrig, ch=keepHistory)
