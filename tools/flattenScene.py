...
def cleanup(verbose=True):
    ...
    for attr in attrList:
        ...
        # For Each Node
        attrNodes = cmds.ls('*.' + attr, o=True, r=True)
...
def deleteNodes(verbose=True):
    ...
    # Check Set Exists
    if not cmds.objExists('nodesToDelete'): return []

    # Check Nodes
    nodesToDelete = cmds.sets('nodesToDelete', q=True)
...
def renameOnFlatten(verbose=True):
    ...
    nodeList = cmds.ls('*.renameOnFlatten', o=True)
    ...
    for node in nodeList:
        ...
        # Get Rename String
        renameStr = cmds.getAttr(node + '.renameOnFlatten')
        # Check Empty String
        if not renameStr: continue
        if cmds.objExists(renameStr):
            raise Exception(
                'RenameOnFlatten: Object "' + node + '" cant be renamed to "' + renameStr + '"! Object of that name already exists...')

        # Rename Node
        renamed = cmds.rename(node, renameStr)
...
def reparentOnFlatten(verbose):
    ...
    nodeList = cmds.ls('*.reparentOnFlatten', o=True)
    if not nodeList: return []
    ...
    for node in nodeList:
        ...
        # Get Parent
        parentConn = cmds.listConnections(node + '.reparentOnFlatten', s=True, d=False)
        ...
        # Reparent Node
        reparented = cmds.parent(node, parent)[0]
...
def deleteHistory(verbose):
    ...
    nodeList = cmds.ls('*.deleteHistoryOnFlatten', o=True, dag=True)
    if not nodeList: return []
    ...
    for node in nodeList:
        ...
        try:
            ...
            print('Problem deleting construction history on node "' + node + '" during flattenScene!')
...
def fixNonReferenceInputs(verbose=False):
    ...
    """
    # Get Root Nodes
    rootNodes = cmds.ls('*.fixNonReferenceInputsRoot', o=True, r=True) or []
...
def encodeReferenceFilePath(verbose=True):
    ...
    nodeList = cmds.ls('*.encodeReferenceFilePath', o=True, r=True)
    if not nodeList: return []
    ...
    for node in nodeList:
        ...
        cmds.setAttr(node + '.' + attr, realPath, type='string')
...
def createNodesToDeleteSet(nodes=[]):
    ...
    if not cmds.objExists(nodesToDeleteSet):
        cmds.sets(n=nodesToDeleteSet, empty=True)

    # Add Items to Set
    if nodes: cmds.sets(nodes, fe=nodesToDeleteSet)
...
def addRenameAttr(node, rename=''):
    ...
    if cmds.objExists(node + '.renameOnFlatten'):
    ...
    # Set Rename Value
    if rename: cmds.setAttr(node + '.' + attr, rename, type='string')
...
def addReparentAttr(node, parent=''):
    ...
    if parent:
        try:
            cmds.connectAttr(parent + '.message', node + '.' + attr, f=True)
        except:
            pass
...
def addReparentAttrFromSel():
    ...
    if len(sel) < 2:
        raise Exception(
            'Invalid selection for addReparentAttrFromSel()! Select the child and parent object and run again.')
