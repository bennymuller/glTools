...
def measureAttach(measureList, baseMesh, detachList=[]):
    ...
    for obj in detachList:
        ...
        meshCon = cmds.listConnections(obj, type='dnMeshConstraint')
        if meshCon: cmds.delete(meshCon)
...
def extractExpressionRegions(char, deleteHistory=True, rebuildDeltas=False):
    ...
    cmds.file(regionPath, i=True, type='mayaBinary', rpr="_", options='v=0', pr=False)
    ...
    expressionList = [i.split('_')[0] for i in cmds.listRelatives(expressionGrp, c=True)]

    # Identify region list
    regionList = cmds.listRelatives(face_regionExtract_grp, c=True, type='joint')
    ...
    if rebuildDeltas and not deleteHistory:
        ...
        for target in expressionList:
            # Reset delata shape
            reset_blendShape = cmds.blendShape(face_mesh, target + '_delta', n=target + '_reset_blendShape')[0]
            cmds.setAttr(reset_blendShape + '.' + face_mesh, 1.0)
            cmds.delete(target + '_delta', ch=True)

            # Re-apply deltas as result of rigBase and sculpt
            delta_blendShape = \
            cmds.blendShape([target + '_rigBase', target + '_sculpt'], target + '_delta', n=target + '_delta_blendShape')[
                0]
            cmds.setAttr(delta_blendShape + '.' + target + '_rigBase', -1.0)
            cmds.setAttr(delta_blendShape + '.' + target + '_sculpt', 1.0)
    ...
    for target in expressionList:
        extractList = glTools.tools.shapeExtract.shapeExtract_skinCluster(face_mesh, target + '_delta', skinCluster,
                                                                          regionList, deleteHistory, prefix=target)
        expressionExtractList += extractList

    # Group extractin results
    expressionExtractGrp = cmds.ls(cmds.listRelatives(expressionExtractList, p=True))
    expressionExtractMainGrp = cmds.group(expressionExtractGrp, n='expression_extract_grp')

    # -------------------------------------
    # - Apply Extracted Expression Deltas -
    # -------------------------------------

    region_blendShape = cmds.blendShape(expressionExtractList, face_mesh, n=region_blendShape)[0]
    region_blend_mesh = cmds.rename(face_mesh, region_blend_mesh)
    cmds.parent(region_blend_mesh, w=True)
    ...
    cmds.delete('*_curveInfo')
    cmds.delete('*_detachCurve')
    cmds.delete(face_mesh_grp)

    # Delete region mesh and joints
    cmds.delete(face_regionExtract_grp)

    # Check deleteHistory
    if deleteHistory:
        # Delete region extract meshes
        cmds.delete(expressionExtractMainGrp)

        # Delete expression mesh group
        cmds.delete(expressionGrp)
...
def rebuildExpressionsWithLatestRig(char, rigHighDetail=True):
    ...
    if rigHighDetail:
        ...
        if cmds.objExists(highDetailAttr):
            cmds.setAttr(highDetailAttr, 1)

    # Turn Off Expression Detail
    expressionDetailAttr = basePrefNull + '.expressionShape'
    if cmds.objExists(expressionDetailAttr):
        cmds.setAttr(expressionDetailAttr, 0)
    ...
    widthMin = cmds.getAttr(faceMesh + '.boundingBoxMinX')
    widthMax = cmds.getAttr(faceMesh + '.boundingBoxMaxX')
    width = (widthMax - widthMin) * 1.1

    heightMin = cmds.getAttr(faceMesh + '.boundingBoxMinY')
    heightMax = cmds.getAttr(faceMesh + '.boundingBoxMaxY')
    ...
    mel.eval(
        'dnAnim -read -namespace "' + fRig_ns + ':" -keyed 1 -unkeyed 1 -stripNamespaces 1 -matchMode "partialPath" -file "' + animPath + '"')
    ...
    for i in range(len(expressionList)):
        ...
        cmds.currentTime(20 * (i + 1))

        # Duplicate expression result
        rigBaseNew = cmds.duplicate(faceMesh, rr=True, n=expressionList[i] + '_rigBaseNEW')[0]

        # Shift expression mesh along the X axis
        cmds.move(width * (i + 1), 0, 0, rigBaseNew)

        # Get expression rigBase
        rigBase = expressionList[i] + '_rigBase'
        if cmds.objExists(rigBase):
            rigBase = cmds.rename(rigBase, rigBase + 'OLD')
        else:
            raise Exception('Cant find RIGBASE for ' + expressionList[i] + ' expression!!')

        # Move rigBase
        cmds.move(width * (i + 1), 0, 0, rigBase)
    ...
    rigBaseGrp = cmds.group(rigBaseList, n='expression_rigBaseOLD_GRP', w=True)
    rigBaseNewGrp = cmds.group(animResultList, n='expression_rigBaseNEW_GRP', w=True)

    cmds.move(0, -height * 4, 0, rigBaseNewGrp)
    cmds.move(0, -height * 3, 0, rigBaseGrp)
    ...
    for i in range(len(deltaList)):
        cmds.blendShape(addBlendShape, e=True, t=(faceMesh, targetIndex + i, deltaList[i], 1.0))
    ...
    for i in range(len(deltaList)):
        cmds.setKeyframe(addBlendShape, attribute=deltaList[i], t=(20 * i), v=0.0)
        cmds.setKeyframe(addBlendShape, attribute=deltaList[i], t=(20 * (i + 1)), v=1.0)
        cmds.setKeyframe(addBlendShape, attribute=deltaList[i], t=(20 * (i + 2)), v=0.0)
    ...
    for i in range(len(deltaList)):
        ...
        # Duplicate expression result
        expressionResult = cmds.duplicate(faceMesh, rr=True, n=expressionList[i] + '_sculptNEW')[0]

        # Shift expression mesh along the X axis
        cmds.move(width * (i + 1), 0, 0, expressionResult)

        # Layout expression sculpt and concept
        expressionSculpt = expressionList[i] + '_sculpt'
        cmds.move(width * (i + 1), 0, 0, expressionSculpt)
        expressionConcept = expressionList[i] + '_concept'
        cmds.move(width * (i + 1), 0, 0, expressionConcept)
    ...
    # Group and position expression mesh results
    expressionResultGrp = cmds.group(expressionResultList, n='expression_sculptNEW_GRP', w=True)
    expressionSculptGrp = cmds.group(expressionSculptList, n='expression_sculptOLD_GRP', w=True)
    expressionConceptGrp = cmds.group(expressionConceptList, n='expression_concept_GRP', w=True)

    cmds.move(0, height, 0, expressionConceptGrp)
    cmds.move(0, -height, 0, expressionSculptGrp)
    ...
    expressionDetailAttr = basePrefNull + '.expressionShape'
    if cmds.objExists(expressionDetailAttr):
        cmds.setAttr(expressionDetailAttr, 1)
    ...
    cmds.move(-width, 0, 0, expressionGrp)
    cmds.setAttr(expressionGrp + '.v', 0)
    cmds.move(-width * 2, 0, 0, conceptGrp)
    cmds.setAttr(conceptGrp + '.v', 0)
...
def replaceExpressionDeltaMesh(namespace='TARSFACEa01:'):
    ...
    connectionList = cmds.listConnections(sourceBlendShape + '.w', s=True, p=True, c=True)
    for i in range(0, len(connectionList), 2):
        targetPlug = connectionList[i].replace(sourceBlendShape, targetBlendShape)
        cmds.connectAttr(connectionList[i + 1], targetPlug, f=True)
...
def barycentricPointWeightSetup(samplePt, targetList, calcList=[True, True, True], prefix=''):
    ...
    for target in targetList:
        ...
        targetLoc.append(cmds.spaceLocator(n=prefix + '_' + targetSuffix + '_pnt')[0])
        cmds.setAttr(targetLoc[-1] + '.t', targetPos[0], targetPos[1], targetPos[2])
...
def combinationShape(attr1, attr2, invertAttr1, invertAttr2, driveAttr, prefix):
    ...
    """
    # Checks
    if not cmds.objExists(attr1):
        raise Exception('Input attribute "' + attr1 + '" does not exist!!')
    if not cmds.objExists(attr2):
        raise Exception('Input attribute "' + attr2 + '" does not exist!!')

    # Check Invert
    revNode = ''
    if invertAttr1 or invertAttr2:
        revNode = cmds.createNode('reverse', n=prefix + '_combine_reverse')

    # Create comination multDoubleLinear node
    multNode = cmds.createNode('multDoubleLinear', n=prefix + '_combine_multDoubleLinear')

    # Connect Attr 1
    if invertAttr1:
        cmds.connectAttr(attr1, revNode + '.inputX', f=True)
        cmds.connectAttr(revNode + '.outputX', multNode + '.input1', f=True)
    else:
        cmds.connectAttr(attr1, multNode + '.input1', f=True)

    # Connect Attr 2
    if invertAttr2:
        cmds.connectAttr(attr2, revNode + '.inputY', f=True)
        cmds.connectAttr(revNode + '.outputY', multNode + '.input2', f=True)
    else:
        cmds.connectAttr(attr2, multNode + '.input2', f=True)

    # Connect combined value to blendShape target weight
    cmds.connectAttr(multNode + '.output', driveAttr, f=True)
...
def normalizeShapes(blendShape, targetList, prefix):
    ...
    combine_pma = cmds.createNode('plusMinusAverage', n=prefix + '_combinedValue_plusMinusAverage')
    for i in range(len(targetList)):
        targetConn = cmds.listConnections(blendShape + '.' + targetList[i], s=True, d=False, p=True)
        cmds.connectAttr(targetConn[0], combine_pma + '.input1D[' + str(i) + ']', f=True)

    # Check combined value
    combine_cond = cmds.createNode('condition', n=prefix + '_combinedValue_condition')
    cmds.setAttr(combine_cond + '.operation', 2)  # Greater Than
    cmds.setAttr(combine_cond + '.secondTerm', 1.0)
    cmds.connectAttr(combine_pma + '.output1D', combine_cond + '.firstTerm', f=True)
    # Set Division Factor
    cmds.setAttr(combine_cond + '.colorIfFalseR', 1.0)
    cmds.connectAttr(combine_pma + '.output1D', combine_cond + '.colorIfTrueR', f=True)

    # Divide each target value
    for i in range(len(targetList)):
        target_suffix = targetList[i].split('_')[-1]
        targetNorm_mdn = cmds.createNode('multiplyDivide', n=prefix + '_' + target_suffix + 'Normalize_multiplyDivide')
        targetConn = cmds.listConnections(blendShape + '.' + targetList[i], s=True, d=False, p=True)
        cmds.connectAttr(targetConn[0], targetNorm_mdn + '.input1X', f=True)
        cmds.connectAttr(combine_cond + '.outColorR', targetNorm_mdn + '.input2X', f=True)
        cmds.setAttr(targetNorm_mdn + '.operation', 2)  # Divide
        cmds.connectAttr(targetNorm_mdn + '.outputX', blendShape + '.' + targetList[i], f=True)
...
def cleanBlendShape(blendShape, baseGeometry):
    ...
    for target in targetList:
        ...
        if not targetConn:
            try:
                glTools.utils.blendShape.removeTarget(blendShape, target, baseGeometry)
            except:
                continue
