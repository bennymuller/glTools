...
def writeGeoCache(path, name, mesh, startFrame, endFrame, pad=4, uvSet='', worldSpace=True, gz=False):
    ...
    if uvSet:
        ...
        uvSet = str(cmds.polyUVSet(mesh, q=True, cuv=True)[0])
    ...
    for f in range(startFrame, endFrame + 1):
        ...
        for i in range(vtxArray.length()):
            FILE.write(str(vtxArray[i].x) + ' ' + str(vtxArray[i].y) + ' ' + str(vtxArray[i].z) + ' ' + str(
                vtxArray[i].w) + ' (' + str(normArray[i].x) + ' ' + str(normArray[i].y) + ' ' + str(
                normArray[i].z) + ')\n')
    ...
    for f in range(startFrame, endFrame + 1):
        ...
        while not faceIter.isDone():
            ...
            for i in range(vtxCount):
                ...
                # UV Id
                try:
                    faceIter.getUVIndex(i, uvIdPtr, uvSet)
                except:
                    pass
                else:
                    faceData += str(OpenMaya.MScriptUtil(uvIdPtr).asInt() + 1)
                faceData += '/'
