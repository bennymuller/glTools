...
def safeImportScene(sceneFile=None):
    ...
    if sceneFile == None:
        startDir = cmds.workspace(q=True, dir=True)
        sceneFile = cmds.fileDialog2(fileFilter='Maya Files (*.ma *.mb)',
                                   dialogStyle=2,
                                   fileMode=1,
    ...
    importNSinc = ''
    while cmds.namespace(ex=importNS + str(importNSinc)):
        if not importNSinc: importNSinc = 0
        importNSinc += 1
    importNS = importNS + str(importNSinc)

    # Import Scene File
    importNodes = cmds.file(sceneFile, i=True, type=fileType[fileExtension], namespace=importNS, preserveReferences=True,
                          returnNewNodes=True)
    ...
    importNsList = cmds.namespaceInfo(importNS, listOnlyNamespaces=True) or []
    ...
    for nestedNS in importNsList:
        ...
        if cmds.namespace(ex=NS):
            ...
            try:
                safeNSind = int(safeNSsuffix)
            except:
                safeNSind = 1
            else:
                safeNS = safeNS.replace('_' + safeNSsuffix, '')
            while cmds.namespace(ex=safeNS + '_%03d' % safeNSind): safeNSind += 1
            ...
            try:
                ...
                cmds.file(refFile, e=True, namespace=safeNS)
            except Exception, e:
...
def fixNamespaceClashes():
    ...
    """
    NSlist = [x for x in cmds.namespaceInfo(listOnlyNamespaces=True, recurse=True) if not x in ['UI', 'shared']]
