...
class NamingConvention(object):
    def __init__(self):
        ...
        self.node['aicmdsonstraint'] = 'acmds'
        ...
        self.node['geometryConstraint'] = 'gcmds'
    ...
    def getName(self, side='', part='', optSide='', subPart='', node='', optSideIndex=1, partIndex=1, subPartIndex=1):
        ...
        if optSide:
            if not self.side.has_key(optSide): raise UserInputError(
                'Invalid otional side ("' + optSide + '") provided!')
        if subPart:
            if not self.subPart.has_key(subPart): raise UserInputError(
                'Invalid sub-part name ("' + subPart + '") provided!')
    ...
    def isValid(self, name):
        ...
        if nameStrCount < self.elecmdsount[0]:
            print 'Name element count (' + str(nameStrCount) + ') is lower than the minimum (' + str(
                self.elecmdsount[0]) + ') expected by the naming conventions!'
            return False
        if nameStrCount > self.elecmdsount[1]:
            print 'Name element count (' + str(nameStrCount) + ') is higher than the maximum (' + str(
                self.elecmdsount[1]) + ') expected by the naming conventions (3-5)!'
            return False
        ...
        if nameStrCount > 3:
            if nameElem[2] != re.search(self.namePattern[2], nameElem[2]).group(0):
                print 'OptinalSide/SubPart string ("' + nameElem[
                    2] + '") does not match naming convention pattern (xx00)!'
                return False
        if nameStrCount > 4:
            if nameElem[3] != re.search(self.namePattern[2], nameElem[3]).group(0):
                print 'OptinalSide/SubPart string ("' + nameElem[
                    3] + '") does not match naming convention pattern (xx00)!'
                return False
        # Check suffix
    ...
    def appendName(self, name='', appendString='', stripNameSuffix=False, trimName=False):
        ...
        if trimName and (nameStrCount + appendStrCount) > self.elecmdsount[1]:
            maxNameIndex = self.elecmdsount[1] - appendStrCount
        else:
            maxNameIndex = nameStrCount
