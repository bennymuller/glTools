...
def checkNonReferenceInputShapes(root, verbose=False):
    ...
    xforms = cmds.ls(cmds.listRelatives(root, ad=True), transforms=True)
    geoList = [geo for geo in xforms if cmds.listRelatives(geo, s=True)]
...
def fixNonReferenceInputShapes(root, verbose=False):
    ...
    for geo in nonRefInputs:
        ...
        try:
            glTools.rig.utils.fixNonReferencedInputShape(geo)
        except Exception, e:
            print(e)
    if verbose:
        print('======================================')
