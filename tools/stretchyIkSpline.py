...
def stretchyIkSpline_parametric(ikHandle,
    ...
    # Check scaleAttr
    scale = cmds.objExists(scaleAttr)
    ...
    ik_curve = cmds.listConnections(ikHandle + '.inCurve', s=True, d=False, sh=True)[0]

    # Get curve parameter information
    minu = cmds.getAttr(ik_curve + '.minValue')
    maxu = cmds.getAttr(ik_curve + '.maxValue')
    udist = maxu - minu
    ...
    for i in range(len(ik_joints)):
        ...
        poc = cmds.createNode('pointOnCurveInfo', n=prefix + ind + '_pointOnCurveInfo')
        cmds.connectAttr(ik_curve + '.worldSpace[0]', poc + '.inputCurve', f=True)
        # Determine Parameter Value
        if useClosestPoint:
            uValue = glTools.utils.curve.closestPoint(ik_curve, ik_joints[i])
            cmds.setAttr(poc + '.parameter', uValue)
        else:
            cmds.setAttr(poc + '.parameter', minu + inc * i)
    ...
    for i in range(len(ik_joints) - 1):
        ind = str(i + 1)
        if i < 9: ind = '0' + ind
        dist = cmds.createNode('distanceBetween', n=prefix + ind + '_distanceBetween')
        distNodeList.append(dist)
        cmds.connectAttr(pointOnCurveList[i] + '.position', dist + '.point1', f=True)
        cmds.connectAttr(pointOnCurveList[i + 1] + '.position', dist + '.point2', f=True)
    ...
    for i in range(len(ik_joints) - 1):
        ...
        md = cmds.createNode('multiplyDivide', n=prefix + ind + '_multiplyDivide')
        multNodeList.append(md)
        cmds.setAttr(md + '.operation', 2)  # Divide
        cmds.connectAttr(distNodeList[i] + '.distance', md + '.input1X', f=True)
        cmds.connectAttr(ik_joints[i + 1] + '.t' + scaleAxis, md + '.input2X', f=True)

        # Create blend node
        blendNode = ''
        if blend:
            blendNode = cmds.createNode('blendTwoAttr', n=prefix + ind + '_blendTwoAttr')
        ...
        if scale:
            ...
            scaleNode = cmds.createNode('multiplyDivide', n=prefix + ind + '_multiplyDivide')
            scaleNodeList.append(scaleNode)
            cmds.setAttr(scaleNode + '.operation', 2)  # Divide
            cmds.connectAttr(md + '.outputX', scaleNode + '.input1X', f=True)
            cmds.connectAttr(scaleAttr, scaleNode + '.input2X', f=True)
            # Connect to joint scale
            if blend:
                cmds.connectAttr(scaleNode + '.outputX', blendNode + '.input[1]', f=True)
                cmds.connectAttr(blendNode + '.output', ik_joints[i] + '.s' + scaleAxis, f=True)
            else:
                cmds.connectAttr(scaleNode + '.outputX', ik_joints[i] + '.s' + scaleAxis, f=True)
        else:
            # Connect to joint scale
            if blend:
                cmds.connectAttr(md + '.outputX', blendNode + '.input[1]', f=True)
                cmds.connectAttr(blendNode + '.output', ik_joints[i] + '.s' + scaleAxis, f=True)
            else:
                cmds.connectAttr(md + '.outputX', ik_joints[i] + '.s' + scaleAxis, f=True)
