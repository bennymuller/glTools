...
class ConnectionAttrStorage(object):
    ...
    def getParent(self):
        ...
        for node in self.nodes:
            ...
            parent = cmds.listRelatives(node, allParents=True)[0]
    ...
    def getIncomingConnections(self):
        ...
        for node in self.nodes:
            connections = cmds.listConnections(node, plugs=True, source=True, destination=False, connections=True,
    ...
    def getOutgoingConnections(self):
        ...
        for node in self.nodes:
            connections = cmds.listConnections(node, plugs=True, source=False, destination=True, connections=True,
                                             skipConversionNodes=True)
    ...
    def createAttrs(self):
        ...
        for node in self.connectionStorage.keys():
            if not cmds.objExists('%s.%s' % (node, self.storageAttr)):
                cmds.addAttr(node, ln=self.storageAttr, dataType='string', en=':Keyable:NonKeyable:Locked:', m=1)
    ...
    def setAttrs(self):
        ...
        for node in self.connectionStorage.keys():
            for i, connection in enumerate(self.connectionStorage[node]):
                cmds.setAttr('%s.%s[%s]' % (node, self.storageAttr, i), str(connection), type='string')
    ...
    def getAttrs(self):
        ...
        for node in self.nodes:
            ...
            for index in cmds.getAttr('%s.%s' % (node, self.storageAttr), multiIndices=True):
                self.connectionStorage[node].append(eval(cmds.getAttr('%s.%s[%s]' % (node, self.storageAttr, index))))
