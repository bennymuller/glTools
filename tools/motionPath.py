...
def createCurve(pt, start=None, end=None, inc=1):
    ...
    crv = cmds.curve(p=[pos], d=1)
    crv = cmds.rename(crv, pt + '_curve')
    ...
    cmds.rebuildCurve(crv, ch=False, rpo=True, rt=0, end=1, kr=2, kcp=True, kep=True)
    ...
    cmds.addAttr(crv, ln='sourceObject', dt='string')
    cmds.setAttr(crv + '.sourceObject', pt, type='string')

    # Encode Motion Start/End
    cmds.addAttr(crv, ln='motionStart')
    cmds.setAttr(crv + '.motionStart', start)
    cmds.addAttr(crv, ln='motionEnd')
    cmds.setAttr(crv + '.motionEnd', end)
...
def createCurves(ptList, start=None, end=None, inc=1):
    ...
    for i in range(start + inc, end + 1, inc):
        ...
        for n in range(len(ptList)):
            ...
            cmds.curve(crvList[n], a=True, p=pos)
...
def createAverageCurve(xformList, start=None, end=None, inc=1, prefix=''):
    ...
    # Xform list
    xformList = cmds.ls(xformList, transforms=True) or []
    ...
    loc = cmds.group(em=True, n=prefix)
    cmds.pointConstraint(xformList, loc, mo=False)
...
def motionPathToKeys(path, obj=None, start=None, end=None):
    ...
    if not obj:
        if not cmds.attributeQuery('sourceObject', n=path, ex=True):
            raise Exception('No keyframe object specified!')
        else:
            obj = cmds.getAttr(path + '.sourceObject')
            if not cmds.objExists(path):
                raise Exception('Motion path curve "' + path + '" does not exist!')

    # Check Start/End
    if start == None:
        if cmds.attributeQuery('motionStart', n=path, ex=True):
            start = cmds.getAttr(path + '.motionStart')
        else:
            start = cmds.playbackOptions(q=True, min=True)
    if end == None:
        if cmds.attributeQuery('motionEnd', n=path, ex=True):
            end = cmds.getAttr(path + '.motionEnd')
        else:
    ...
    objMatrix = glTools.utils.matrix.fromList(cmds.getAttr(obj + '.parentMatrix'))
