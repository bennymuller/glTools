...
def pointConstraint(master, slave, mo=False, attrList=['tx', 'ty', 'tz']):
    ...
    # Check Settable Channels
    st = []
    if not 'tx' in attrList or not cmds.getAttr(slave + '.tx', se=True): st.append('x')
    if not 'ty' in attrList or not cmds.getAttr(slave + '.ty', se=True): st.append('y')
    if not 'tz' in attrList or not cmds.getAttr(slave + '.tz', se=True): st.append('z')
    if not st: st = 'none'
    ...
    constraint = ''
    try:
        constraint = cmds.pointConstraint(master, slave, sk=st, mo=mo)[0]
    except Exception, e:
...
def orientConstraint(master, slave, mo=False, attrList=['rx', 'ry', 'rz']):
    ...
    # Check Settable Channels
    sr = []
    if not 'rx' in attrList or not cmds.getAttr(slave + '.rx', se=True): sr.append('x')
    if not 'ry' in attrList or not cmds.getAttr(slave + '.ry', se=True): sr.append('y')
    if not 'rz' in attrList or not cmds.getAttr(slave + '.rz', se=True): sr.append('z')
    if not st: st = 'none'
    ...
    constraint = ''
    try:
        constraint = cmds.orientConstraint(master, slave, sk=sr, mo=mo)[0]
    except Exception, e:
        raise Exception('Error creating constraint from "' + master + '" to "' + slave + '"! Exception msg: ' + str(e))
...
def parentConstraint(master, slave, mo=False, attrList=['tx', 'ty', 'tz', 'rx', 'ry', 'rz']):
    ...
    constraint = ''
    try:
        constraint = cmds.parentConstraint(master, slave, st=st, sr=sr, mo=mo)[0]
    except Exception, e:
        raise Exception('Error creating constraint from "' + master + '" to "' + slave + '"! Exception msg: ' + str(e))
...
def nonReferencedConstraints(slaveNSfilter=None, targetNSfilter=None):
    ...
    sceneConstraints = cmds.ls(type='constraint')
...
def translateOffsetTarget(target, offset, slave, prefix=None):
    ...
    cmds.delete(parentConstraint(target, slave))
    cmds.parent(slave, target)

    # Create Offset Constraint
    offsetConstraint = cmds.pointConstraint(offset, slave, mo=True)[0]
    offsetConstraint = cmds.rename(offsetConstraint, prefix + '_offset_pointConstraint')
