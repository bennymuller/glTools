...
def matchAttrs(src, dst, attrList=None):
    ...
    for attr in attrList:
        ...
        if not cmds.attributeQuery(attr, n=src, ex=True):
            print('Source attribute "' + src + '.' + attr + '" not found! Skipping...')
            continue
        if not cmds.attributeQuery(attr, n=dst, ex=True):
            print('Destination attribute "' + dst + '.' + attr + '" not found! Skipping...')
            continue
        if not cmds.getAttr(dst + '.' + attr, se=True):
        ...
        try:
            cmds.setAttr(dst + '.' + attr, val)
        except Exception, e:
            print('Error setting destination attribute "' + dst + '.' + attr + '"! Exception Msg: ' + str(e))
...
class Match(object):
    ...
    def setTwinMatchAttrs(self, masterList, pivot, axis='x', mode=0, search='lf_', replace='rt_'):
        ...
        for master in masterList:
            if not cmds.objExists(master): raise Exception('Control object ' + master + ' does not exist!!')
            twin = master
    ...
    def setModeAttr(self, controlList, mode):
        ...
        for control in controlList:
            if cmds.objExists(control + '.matchMode'):
                cmds.setAttr(control + '.matchMode', mode)
            else:
                print(
                'Control object ' + control + ' does not have a "matchMode" attribute! Run Match.setMatchAttrs()!!')
    ...
    def setMatchAttrs(self, master, twin, pivot, axis, mode=0, twinX='', twinY='', twinZ=''):
        ...
        # Check inputs
        if not cmds.objExists(master): raise Exception('Master object ' + master + ' does not exist')
        if not cmds.objExists(twin): raise Exception('Twin object ' + twin + ' does not exist')
        if not cmds.objExists(pivot): raise Exception('Pivot object ' + pivot + ' does not exist')

        # Delete Compound Attr
        if cmds.objExists(master + '.' + self.parentAttr):
            cmds.deleteAttr(master + '.' + self.parentAttr)
        ...
        # Parent Attribute
        cmds.addAttr(master, ln=self.parentAttr, numberOfChildren=7, at='compound')
        # Twin
        cmds.addAttr(master, ln=self.twinAttr, dt='string', p=self.parentAttr)
        # Pivot
        cmds.addAttr(master, ln=self.pivotAttr, dt='string', p=self.parentAttr)
        # Axis
        cmds.addAttr(master, ln=self.axisAttr, at='enum', en="X:Y:Z", p=self.parentAttr)
        # Mode
        cmds.addAttr(master, ln=self.modeAttr, at='enum', en="World:Local:Match", p=self.parentAttr)

        # - Twin XYZ -

        # X Axis
        cmds.addAttr(master, ln=self.twinXAttr, at='enum', en="X:Y:Z:-X:-Y:-Z", p=self.parentAttr)
        # Y Axis
        cmds.addAttr(master, ln=self.twinYAttr, at='enum', en="X:Y:Z:-X:-Y:-Z", p=self.parentAttr)
        # Z Axis
        cmds.addAttr(master, ln=self.twinZAttr, at='enum', en="X:Y:Z:-X:-Y:-Z", p=self.parentAttr)

        # ==================
        # - Set Attributes -
        # ==================

        cmds.setAttr(master + '.' + self.parentAttr + '.' + self.twinAttr, twin, type='string')
        cmds.setAttr(master + '.' + self.parentAttr + '.' + self.pivotAttr, pivot, type='string')
        cmds.setAttr(master + '.' + self.parentAttr + '.' + self.axisAttr, self.axisIndex[axis])
        cmds.setAttr(master + '.' + self.parentAttr + '.' + self.modeAttr, mode)

        # - Twin XYZ -
        twinAttrVal = self.getTwinMirrorAxis(master, twin, pivot, axis)

        if twinX:
            cmds.setAttr(master + '.' + self.parentAttr + '.' + self.twinXAttr, self.axisIndex[twinX])
        else:
            cmds.setAttr(master + '.' + self.parentAttr + '.' + self.twinXAttr, twinAttrVal[0])
        if twinY:
            cmds.setAttr(master + '.' + self.parentAttr + '.' + self.twinYAttr, self.axisIndex[twinY])
        else:
            cmds.setAttr(master + '.' + self.parentAttr + '.' + self.twinYAttr, twinAttrVal[1])
        if twinZ:
            cmds.setAttr(master + '.' + self.parentAttr + '.' + self.twinZAttr, self.axisIndex[twinZ])
        else:
            cmds.setAttr(master + '.' + self.parentAttr + '.' + self.twinZAttr, twinAttrVal[2])
    ...
    def twinTransform(self,
        ...
        # Check Twin
        if not slave:
            if cmds.attributeQuery(self.twinAttr, n=master, ex=True):
                slave = cmds.getAttr(master + '.' + self.twinAttr)
            else:
        ...
        # Check Pivot
        if not pivot:
            if cmds.attributeQuery(self.pivotAttr, n=master, ex=True):
                pivot = cmds.getAttr(master + '.' + self.pivotAttr)
            else:
        ...
        # Check Mirror Axis
        if not mirrorAxis:
            if cmds.attributeQuery(self.axisAttr, n=master, ex=True):
                mirrorAxis = cmds.getAttr(master + '.' + self.axisAttr)
            else:
        ...
        # Check Mirror Mode
        if not mirrorMode:
            if cmds.attributeQuery(self.modeAttr, n=master, ex=True):
                mirrorMode = cmds.getAttr(master + '.' + self.modeAttr)
            else:
        ...
        masterParent = None
        try:
            masterParent = str(cmds.listRelatives(master, p=1)[0])
        except:
            pass
        slaveParent = None
        try:
            slaveParent = str(cmds.listRelatives(slave, p=1)[0])
        except:
            pass
        pivotParent = None
        try:
            pivotParent = str(cmds.listRelatives(pivot, p=1)[0])
        except:
            pass

        # Get Match Values
        twinX = cmds.getAttr(slave + '.' + self.twinXAttr)
        twinY = cmds.getAttr(slave + '.' + self.twinYAttr)
        twinZ = cmds.getAttr(slave + '.' + self.twinZAttr)
        slaveRotateOrder = cmds.getAttr(slave + '.ro')
        masterRotateOrder = cmds.getAttr(master + '.ro')

        # Get Mater Transform channel values
        pos = list(cmds.getAttr(master + '.translate')[0])
        rot = list(cmds.getAttr(master + '.rotate')[0])
        scl = list(cmds.getAttr(master + '.scale')[0])
        ...
        if mirrorMode == 0:  # WORLD Match
            ...
            if xformList[0] or xformList[1] or xformList[2]:
                ...
                if master == pivot:
                    ...
                    # Transform to World
                    if masterParent: pos = self.transformVector(pos, masterParent, transformAsPoint=True,
                    ...
                    if slaveParent: pos = self.transformVector(pos, slaveParent, transformAsPoint=True,
    ...
    def twinCustomAttrs(self, master='', slave='', customAttrList=[]):
        ...
        for attr in customAttrList:
            ...
            customAttrValues[twinAttr] = cmds.getAttr(master + '.' + attr)
        return customAttrValues

    def twin(self, master='', slave='', pivot='', mirrorAxis='', mirrorMode='', xformList=[1, 1, 1, 1, 1, 1, 1, 1, 1],
             customAttrList=[]):
        """
        ...
        if not cmds.objExists(master + '.' + self.twinAttr): return
        if not slave: slave = str(cmds.getAttr(master + '.' + self.twinAttr))
        if not pivot: pivot = str(cmds.getAttr(master + '.' + self.pivotAttr))
        ...
        # Check objects exist
        if not cmds.objExists(master): raise Exception('Master object ' + master + ' does not exists!')
        if not cmds.objExists(slave): raise Exception('Slave object ' + slave + ' does not exists!')
        if not cmds.objExists(pivot): raise Exception('Pivot object ' + pivot + ' does not exists!')
        ...
        for i in range(9):
            if dcXformList[i]:
                dcXformList[i] = cmds.getAttr(slave + '.' + self.xform[i], se=True)
        ...
        for i in range(9):
            if dcXformList[i]:
                cmds.setAttr(slave + '.' + self.xform[i], xform[i])
        ...
        if not customAttrList:
            customAttrList = cmds.listAttr(master, keyable=True, userDefined=True)
    ...
    #	if cmds.objExists(slave+'.'+attr):
    #		if cmds.getAttr(slave+'.'+attr, settable=True):
    #			try: cmds.setAttr(slave+'.'+attr,customAttrVals[attr])
    #			except: pass
    ...
    def twinSelection(self):
        ...
        if cmds.objExists(ns + self.evalOrderAttr):
            evalOrderLen = cmds.getAttr(ns + self.evalOrderAttr, s=True)
            evalOrder = [(ns + cmds.getAttr(ns + self.evalOrderAttr + '[' + str(i) + ']')) for i in range(evalOrderLen)]
    ...
    def selectTwin(self):
        ...
        # Set Selection
        if twinList: cmds.select(twinList, r=True)
    ...
    def getTwin(self, obj):
        ...
        if cmds.objExists(ns + obj + '.' + self.twinAttr):

            # Get twin name string
            twin = str(cmds.getAttr(ns + obj + '.' + self.twinAttr))
    ...
    def transformVector(self, vector=[0, 0, 0], transform=OpenMaya.MMatrix.identity, transformAsPoint=False,
                        invertTransform=False):
        """
        ...
        if type(transform) == str or type(transform) == unicode:
            ...
            if type(transform) != OpenMaya.MMatrix:
                raise Exception(
                    '"Transform" input variable is not of expected type! Expecting string or MMatrix, received ' + str(
                        type(transform)) + '!!')

        # Transform vector
        if transformMatrix != OpenMaya.MMatrix.identity:
            if invertTransform:
                vector *= transformMatrix.inverse()
            else:
    ...
    def vectorTwinSpaceMirror(self, vector, master, twin, pivot, axis):
        ...
        twinParent = ''
        try:
            twinParent = str(cmds.listRelatives(twin, p=1)[0])
        except:
            pass
        ...
        if cmds.objectType(twin) == 'joint':
            jointOrient = cmds.getAttr(twin + '.jointOrient')[0]
    ...
    def getTransformValues(self, obj):
        ...
        @type obj: str
        """
        return [cmds.getAttr(obj + '.' + self.xform[i]) for i in range(9)]
    ...
    def setTransformValues(self, obj, xformList=[0, 0, 0, 0, 0, 0, 1, 1, 1]):
        ...
        """
        for i in range(9):
            if cmds.getAttr(obj + '.' + self.xform[i], se=True): cmds.setAttr(obj + '.' + self.xform[i], xformList[i])
    ...
    def getCustomAttrValues(self, obj):
        ...
        """
        customAttrVals = {}
        customAttrList = cmds.listAttr(obj, k=True, ud=True)
        if customAttrList:
            for attr in customAttrList: customAttrVals[attr] = cmds.getAttr(obj + '.' + attr)
    ...
    def setCustomAttrValues(self, obj, customAttrVals={}):
        ...
        for attr in customAttrVals.iterkeys():
            if cmds.objExists(obj + '.' + attr):
                if cmds.getAttr(obj + '.' + attr, settable=True):
                    try:
                        cmds.setAttr(obj + '.' + attr, customAttrVals[attr])
                    except:
                        pass
    ...
    def getCBattrList(self):
        ...
        """
        cbAttrList = cmds.channelBox(self.channelBox, q=True, selectedMainAttributes=True)
