...
class EvaluationOrder(object):
    ...
    def ikReorder(self):
        ...
        ikList = cmds.listRelatives(self.hierarchy.fullName, ad=True, type='ikHandle')
        for ik in ikList:
            ikNode = self.hierarchy.findDependNode(ik)
            # Find ikHandle start joint
            startJoint = cmds.listConnections(ik + '.startJoint', s=True, d=False)[0]
    ...
    def constraintReorder(self, constraintList=[]):
        ...
        if not constraintList:
            constraintList = cmds.listRelatives(self.hierarchy.fullName, ad=True, type='constraint')
        ...
        for constraintNode in constraintList:
            ...
            if self.debug:
                if lowestNode:
                    print 'Lowest node = ' + lowestNode.shortName
                else:
                    print 'Lowest node = None'
    ...
    def addAttribute(self, target):
        ...
        # Create attribute
        if not cmds.objExists(target + '.' + self.attribute):
            cmds.addAttr(target, ln=self.attribute, dt='string', multi=True, h=True)
        else:
    ...
    def setAttr(self, target, intersectList=[], evalOrderList=[]):
        ...
        if not cmds.objExists(target):
            raise Exception('Target object ' + target + ' does not exist!')
        # Create attribute
        if cmds.objExists(target + '.' + self.attribute):
            cmds.deleteAttr(target, at=self.attribute)
        ...
        for i in range(len(evalOrderList)):
            cmds.setAttr(target + '.' + self.attribute + '[' + str(i) + ']', evalOrderList[i], type='string')
