...
class VolumeWeights(object):
    ...
    def createRadiusLocator(self, prefix='', operationMode=0):
        ...
        falloffLoc = cmds.spaceLocator(n='%s_falloffLoc' % prefix)[0]
        influenceVolume = cmds.createNode('renderSphere', n='%s_templateFalloffShape' % prefix, p=falloffLoc)
        ...
        cmds.connectAttr(locAttrName, influenceVolume + '.radius', f=True)
        cmds.connectAttr(locAttrName, falloffLoc + '.localScaleX', f=True)
        cmds.connectAttr(locAttrName, falloffLoc + '.localScaleY', f=True)
        cmds.connectAttr(locAttrName, falloffLoc + '.localScaleZ', f=True)
    ...
    def addOperationAttr(self, transform, operationMode=0):
        ...
        # radius attr
        if not cmds.objExists('%s.%s' % (transform, self.radiusAttrName)):
            cmds.addAttr(transform, ln=self.radiusAttrName, min=0.0001, dv=0.5, k=True)

        # operation attr
        if not cmds.objExists('%s.%s' % (transform, self._operationAttrName)):
            cmds.addAttr(transform, ln=self._operationAttrName, attributeType='enum', enumName=self._operationAttrModes,
                       dv=operationMode, keyable=True)

        # affect attr
        if not cmds.objExists('%s.%s' % (transform, self.affectAttrName)):
            cmds.addAttr(transform, ln=self.affectAttrName, attributeType='float', dv=1, keyable=True)
    ...
    def getOperation(self, transform):
        # attr check
        return cmds.getAttr('%s.%s' % (transform, self._operationAttrName))
        ...
        for geometry in self.geometry:
            ...
            for transform in self.weightTransforms:
                self.weights[geometry][transform] = WeightList(
                    glTools.tools.generateWeights.radialWeights(geometry=geometry,
                                                                center=transform,
                                                                radius=cmds.getAttr(
                                                                    '%s.%s' % (transform, self.radiusAttrName)),
                                                                smooth=self.smooth))

                affect = cmds.getAttr('%s.%s' % (transform, self.affectAttrName))
    ...
    def calculateWeights(self, normalize=False, clamp=True):
        ...
        for geometry in self.weights.keys():
            ...
            for transform in self.weights[geometry].keys():
                ...
                if opNum == 0:
                    addWeights += self.weights[geometry][transform]
                elif opNum == 1:
