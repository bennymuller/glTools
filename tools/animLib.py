...
def loadStaticData(line, targetNS):
    ...
    try:
        ...
        print('AnimLib: Load Static Data: Error setting attribute "' + attrPath + '" to value ' + str(
            value) + '! Exception Msg: ' + str(e))
...
def loadAnimData(animFile, lineID, targetNS, frameOffset=0, infinityOverride=None):
    ...
    for data in animData[5:]:
        ...
        cmds.keyTangent(attrPath, e=True, t=(time, time), lock=lock, itt=ittype, ott=ottype)
...
def applyAnim(targetNS, char, animType, anim):
    ...
    if not cmds.namespace(ex=targetNS):
...
def writePoseFile(poseFile, poseNodes=[]):
    ...
    for poseNode in poseNodes:
        ...
        # Get Settable Attrs
        attrs = []
        attrs.extend(cmds.listAttr(poseNode, k=True) or [])
        attrs.extend(cmds.listAttr(poseNode, cb=True) or [])

        # Remove Locked Attrs
        lock_attrs = cmds.listAttr(poseNode, l=True)
        ...
        for at in attrs:
            ...
            # Check Settable
            if not cmds.getAttr(poseNode + '.' + at, se=True): continue
            v = cmds.getAttr(poseNode + '.' + at)
            if v < 0.000001: v = 0
    ...
    for item in animNodes:
        ...
        for i in range(0, len(channels), 2):
            ...
            for i in range(len(keys)):
                ...
                f.write('    ' + str(keys[i]) + ' ' + str(values[i]) + ' ' + inTan[i] + ' ' + outTan[i] + ' ' + str(
        ...
        for staticChan in staticChans:
            ...
            if not keys and not connected:
                f.write('static ' + attr + ' ' + attr + ' ' + node + ' ' + parent + ' ' + str(
                    cmds.getAttr(staticChan)) + '\n')
...
def writeNotes(noteFile, note=None):
    ...
    if not note:
        ...
        f.write('date:   ' + day[datetime.datetime.weekday(
            dt)] + ' ' + dt.month + ' ' + dt.day + ' ' + dt.hour + ':' + dt.minute + ':' + dt.second + ' PST ' + dt.year)
    else:
        f.write(note)
